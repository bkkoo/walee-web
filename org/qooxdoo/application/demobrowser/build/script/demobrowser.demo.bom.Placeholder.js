...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.bom.Placeholder.13f6b22f61a0.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
;var I=K.getProperty(qx.module.Placeholder.PLACEHOLDER_NAME);if(!I){I=qx.module.Placeholder.__za(K);}
;var J=K.isRendered();var L=I.isRendered();if(J&&!L){K.before(I);}
else if(!J&&L){I.remove();return this;}
;qx.module.Placeholder.__yY(K);}
;}
;return this;}
,__yY:function(O){var R=O.getAttribute(l);var Q=O.getProperty(qx.module.Placeholder.PLACEHOLDER_NAME);var P=O.getStyle(B);var S=parseInt(O.getStyle(m))+2*parseInt(O.getStyle(o));var T=parseInt(O.getStyle(u))+2*parseInt(O.getStyle(w));Q.setHtml(R).setStyles({display:O.getValue()==t?n:v,zIndex:P==f?1:P+1,textAlign:O.getStyle(b),width:(O.getWidth()-S-4)+x,height:(O.getHeight()-T-4)+x,left:O.getOffset().left+x,top:O.getOffset().top+x,fontFamily:O.getStyle(c),fontStyle:O.getStyle(s),fontVariant:O.getStyle(g),fontWeight:O.getStyle(a),fontSize:O.getStyle(h),paddingTop:(parseInt(O.getStyle(u))+2)+x,paddingRight:(parseInt(O.getStyle(o))+2)+x,paddingBottom:(parseInt(O.getStyle(w))+2)+x,paddingLeft:(parseInt(O.getStyle(m))+2)+x});}
,__za:function(U){var V=qxWeb.create(j).setStyles({position:r,color:H,overflow:y,pointerEvents:v});U.setProperty(qx.module.Placeholder.PLACEHOLDER_NAME,V);U.on(E,function(W){var X=W.getProperty(qx.module.Placeholder.PLACEHOLDER_NAME);X.setStyle(D,W.getValue()==t?n:v);}
