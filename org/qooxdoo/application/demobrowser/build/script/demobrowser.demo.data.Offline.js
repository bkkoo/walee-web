...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.data.Offline.d91b95821a0d.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
,properties:{model:{nullable:true,event:c,apply:a}},members:{_storage:null,__DE:null,_applyModel:function(j,k){if(k){k.removeListenerById(this.__DE);k.dispose();this.__DE=null;}
;if(j){this.__DE=j.addListener(b,this.__DF,this);this.__DF();}
else {this._storage.removeItem(this._key);}
;}
,__DF:function(){var l=qx.util.Serializer.toNativeObject(this.getModel());this._storage.setItem(this._key,l);}
...
(function(){var c="qx.bom.Storage",b="html.storage.local",a="html.storage.userdata";qx.Bootstrap.define(c,{statics:{__vF:null,getLocal:function(){if(qx.core.Environment.get(b)){return qx.bom.storage.Web.getLocal();}
...
(function(){var d="local",c="session",b="Storage",a="qx.bom.storage.Web";qx.Bootstrap.define(a,{statics:{__zP:null,__zQ:null,getLocal:function(){if(this.__zP){return this.__zP;}
;return this.__zP=new qx.bom.storage.Web(d);}
,getSession:function(){if(this.__zQ){return this.__zQ;}
;return this.__zQ=new qx.bom.storage.Web(c);}
},construct:function(f){this.__nP=f;}
,members:{__nP:null,getStorage:function(){return window[this.__nP+b];}
,getLength:function(){return this.getStorage(this.__nP).length;}
,setItem:function(g,h){h=qx.lang.Json.stringify(h);try{this.getStorage(this.__nP).setItem(g,h);}
catch(e){throw new Error("Storage full.");}
;}
,getItem:function(j){var k=this.getStorage(this.__nP).getItem(j);if(qx.lang.Type.isString(k)){k=qx.lang.Json.parse(k);}
else if(k&&k.value&&qx.lang.Type.isString(k.value)){k=qx.lang.Json.parse(k.value);}
;return k;}
,removeItem:function(l){this.getStorage(this.__nP).removeItem(l);}
,clear:function(){var m=this.getStorage(this.__nP);if(!m.clear){for(var i=m.length-1;i>=0;i--){m.removeItem(m.key(i));}
;}
else {m.clear();}
;}
,getKey:function(n){return this.getStorage(this.__nP).key(n);}
,forEach:function(o,p){var length=this.getLength();for(var i=0;i<length;i++){var q=this.getKey(i);o.call(p,q,this.getItem(q));}
;}
}});}
)();
(function(){var k="session",j="head",h="none",g="qx.bom.storage.UserData",f="div",e="local",d="qxtest",c="#default#userdata",b="display",a="qx";qx.Bootstrap.define(g,{statics:{__zP:null,__zQ:null,__bV:0,getLocal:function(){if(this.__zP){return this.__zP;}
;return this.__zP=new qx.bom.storage.UserData(e);}
,getSession:function(){if(this.__zQ){return this.__zQ;}
;return this.__zQ=new qx.bom.storage.UserData(k);}
},construct:function(l){this.__zR=document.createElement(f);this.__zR.style[b]=h;document.getElementsByTagName(j)[0].appendChild(this.__zR);this.__zR.addBehavior(c);this.__zS=l;this.__zR.load(this.__zS);this.__eM={};this.__zT={};var m=this.__zR.getAttribute(a+qx.bom.storage.UserData.__bV);while(m!=undefined){m=qx.lang.Json.parse(m);this.__eM[m.key]=m.value;this.__zT[m.key]=a+qx.bom.storage.UserData.__bV;qx.bom.storage.UserData.__bV++;m=this.__zR.getAttribute(a+qx.bom.storage.UserData.__bV);}
;}
,members:{__zR:null,__zS:d,__eM:null,__zT:null,getStorage:function(){return this.__eM;}
,getLength:function(){return Object.keys(this.__eM).length;}
,setItem:function(n,o){if(this.__zT[n]){var p=this.__zT[n];}
else {var p=a+qx.bom.storage.UserData.__bV;qx.bom.storage.UserData.__bV++;}
;var q=qx.lang.Json.stringify({key:n,value:o});this.__zR.setAttribute(p,q);this.__zR.save(this.__zS);this.__eM[n]=o;this.__zT[n]=p;}
,getItem:function(r){return this.__eM[r]||null;}
,removeItem:function(s){var v=this.__zT[s];if(v==undefined){return;}
;this.__zR.removeAttribute(v);qx.bom.storage.UserData.__bV--;delete this.__eM[s];delete this.__zT[s];var u=a+qx.bom.storage.UserData.__bV;if(this.__zR.getAttribute(u)){var w=this.__zR.getAttribute(a+qx.bom.storage.UserData.__bV);this.__zR.removeAttribute(u);this.__zR.setAttribute(v,w);var t=qx.lang.Json.parse(w).key;this.__zT[t]=v;}
;this.__zR.save(this.__zS);}
,clear:function(){for(var x in this.__zT){this.__zR.removeAttribute(this.__zT[x]);}
;this.__zR.save(this.__zS);this.__eM={};this.__zT={};}
,getKey:function(y){return Object.keys(this.__eM)[y];}
,forEach:function(z,A){var length=this.getLength();for(var i=0;i<length;i++){var B=this.getKey(i);z.call(A,B,this.getItem(B));}
;}
}});}
)();
(function(){var a="qx.bom.storage.Memory";qx.Bootstrap.define(a,{statics:{__zP:null,__zQ:null,getLocal:function(){if(this.__zP){return this.__zP;}
;return this.__zP=new qx.bom.storage.Memory();}
,getSession:function(){if(this.__zQ){return this.__zQ;}
;return this.__zQ=new qx.bom.storage.Memory();}
},construct:function(){this.__eM={};}
,members:{__eM:null,getStorage:function(){return this.__eM;}
,getLength:function(){return Object.keys(this.__eM).length;}
,setItem:function(b,c){c=qx.lang.Json.stringify(c);this.__eM[b]=c;}
,getItem:function(d){var e=this.__eM[d];if(qx.lang.Type.isString(e)){e=qx.lang.Json.parse(e);}
;return e;}
,removeItem:function(f){delete this.__eM[f];}
,clear:function(){this.__eM={};}
,getKey:function(g){var h=Object.keys(this.__eM);return h[g];}
