...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.test.UIDeclaration.d535c50e65e4.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
(function(){var l="Last Name",k="Page 1",j="First Name",h="firstname",g="Email",f="Demo loaded",d="icon/16/apps/office-calendar.png",c="Page 3",b="move",a="First Window",z="Simple Form",y="Page 2",x="resize",w="Date",v="win",u="Cancel",t="icon/32/apps/office-address-book.png",s="Header",r="Welcome to your first own Window.<br/>Have fun!",q="Save",o="demobrowser.demo.test.UIDeclaration",p="Company",m="Button",n="Item";qx.Class.define(o,{extend:qx.application.Standalone,members:{main:function(){qx.application.Standalone.prototype.main.call(this);var A={clazz:qx.ui.form.Form,children:[{type:s,label:z},{type:n,label:j,validator:null,name:h,widget:{clazz:qx.ui.form.TextField,required:true,width:200}},{type:n,label:l,widget:{clazz:qx.ui.form.TextField}},{type:n,label:p,widget:{clazz:qx.ui.form.TextField}},{type:n,label:g,widget:{clazz:qx.ui.form.TextField}},{type:n,label:w,widget:{clazz:qx.ui.form.DateField}},{type:m,widget:{clazz:qx.ui.form.Button,label:q}},{type:m,widget:{clazz:qx.ui.form.Button,label:u}}]};var B={clazz:qx.ui.window.Window,layout:{clazz:qx.ui.layout.VBox,spacing:10},layoutProperties:{left:20,top:20},showStatusbar:true,status:f,caption:a,icon:d,id:v,listeners:[{event:b,handler:function(e){this.debug("Moved to: "+e.getData().left+"x"+e.getData().top);}
},{event:x,handler:function(e){this.debug("Resized to: "+e.getData().width+"x"+e.getData().height);}
}],children:[{clazz:qx.ui.basic.Atom,label:r,icon:t,rich:true},{clazz:qx.ui.tabview.TabView,layoutProperties:{flex:1},children:[{clazz:qx.ui.tabview.Page,label:k,layoutProperties:{flex:1},layout:{clazz:qx.ui.layout.Canvas},children:[A]},{clazz:qx.ui.tabview.Page,label:y,layoutProperties:{flex:1}},{clazz:qx.ui.tabview.Page,label:c,layoutProperties:{flex:1}}]}]};this.addFromDefinition(B);this.win.open();}
,addFromDefinition:function(C){var D=this.getRoot();this.__Az(D,C);}
,__Az:function(E,F){var G=new F.clazz();var K=qx.lang.Object.clone(F);delete K.clazz;delete K.layout;delete K.children;delete K.id;delete K.listeners;G.set(K);if(F.id!=null){this[F.id]=G;}
;if(F.layout!=null){var H=new F.layout.clazz();var K=qx.lang.Object.clone(F.layout);delete K.clazz;H.set(K);G.setLayout(H);}
;var J=F.children;if(J!=null){for(var i=0;i<J.length;i++){if(G instanceof qx.ui.form.Form){this.__GP(G,J[i]);}
else {this.__Az(G,J[i]);}
;}
;}
;if(G instanceof qx.ui.form.Form){E.add(new qx.ui.form.renderer.Single(G));}
else {E.add(G,F.layoutProperties);}
;if(F.listeners){for(var i=0;i<F.listeners.length;i++){var I=F.listeners[i];G.addListener(I.event,I.handler,I.context);}
;}
;}
,__GP:function(L,M){if(M.type==="Header"){L.addGroupHeader(M.label);return;}
;var N=new M.widget.clazz();var O=qx.lang.Object.clone(M.widget);delete O.clazz;N.set(O);if(M.type==="Item"){L.add(N,M.label,M.validator,M.name);}
else if(M.type==="Button"){L.addButton(N);}
;}
}});}
)();
(function(){var b="qx.ui.form.Form",a="";qx.Class.define(b,{extend:qx.core.Object,construct:function(){qx.core.Object.call(this);this.__BM=[];this._buttons=[];this._buttonOptions=[];this._validationManager=new qx.ui.form.validation.Manager();this._resetter=this._createResetter();}
,members:{__BM:null,_validationManager:null,_groupCounter:0,_buttons:null,_buttonOptions:null,_resetter:null,add:function(c,d,e,name,f,g){if(this.__BN()){this.__BM.push({title:null,items:[],labels:[],names:[],options:[],headerOptions:{}});}
;this.__BM[this._groupCounter].items.push(c);this.__BM[this._groupCounter].labels.push(d);this.__BM[this._groupCounter].options.push(g);if(name==null){name=d.replace(/\s+|&|-|\+|\*|\/|\||!|\.|,|:|\?|;|~|%|\{|\}|\(|\)|\[|\]|<|>|=|\^|@|\\/g,a);}
;this.__BM[this._groupCounter].names.push(name);this._validationManager.add(c,e,f);this._resetter.add(c);}
,addGroupHeader:function(h,k){if(!this.__BN()){this._groupCounter++;}
;this.__BM.push({title:h,items:[],labels:[],names:[],options:[],headerOptions:k});}
,addButton:function(l,m){this._buttons.push(l);this._buttonOptions.push(m||null);}
,__BN:function(){return this.__BM.length===0;}
...
,getGroups:function(){return this.__BM;}
,getButtons:function(){return this._buttons;}
,getButtonOptions:function(){return this._buttonOptions;}
,getItems:function(){var o={};for(var i=0;i<this.__BM.length;i++){var p=this.__BM[i];for(var j=0;j<p.names.length;j++){var name=p.names[j];o[name]=p.items[j];}
;}
;return o;}
,_createResetter:function(){return new qx.ui.form.Resetter();}
},destruct:function(){this.__BM=this._buttons=this._buttonOptions=null;this._validationManager.dispose();this._resetter.dispose();}
});}
)();
(function(){var j="complete",h="qx.event.type.Event",g="value instanceof Function || qx.Class.isSubClassOf(value.constructor, qx.ui.form.validation.AsyncValidator)",f="qx.ui.form.validation.Manager",d="This field is required",c="qx.event.type.Data",b="String",a="";qx.Class.define(f,{extend:qx.core.Object,construct:function(){qx.core.Object.call(this);this.__Bx=[];this.__By={};this.setRequiredFieldMessage(qx.locale.Manager.tr(d));}
,events:{"changeValid":c,"complete":h},properties:{validator:{check:g,init:null,nullable:true},invalidMessage:{check:b,init:a},requiredFieldMessage:{check:b,init:a},context:{nullable:true}},members:{__Bx:null,__Bz:null,__By:null,__BA:null,add:function(k,l,m){if(!this.__BF(k)){throw new Error("Added widget not supported.");}
;if(this.__BG(k)&&!k.getValue){if(l!=null){throw new Error("Widgets supporting selection can only be validated "+"in the form validator");}
;}
;var n={item:k,validator:l,valid:null,context:m};this.__Bx.push(n);}
,remove:function(o){var p=this.__Bx;for(var i=0,q=p.length;i<q;i++){if(o===p[i].item){p.splice(i,1);return o;}
;}
;return null;}
,getItems:function(){var r=[];for(var i=0;i<this.__Bx.length;i++){r.push(this.__Bx[i].item);}
;return r;}
,validate:function(){var w=true;this.__BA=true;var t=[];for(var i=0;i<this.__Bx.length;i++){var u=this.__Bx[i].item;var x=this.__Bx[i].validator;t.push(u);if(x==null){var s=this.__BB(u);w=w&&s;this.__BA=s&&this.__BA;continue;}
;var s=this.__BC(this.__Bx[i],u.getValue());w=s&&w;if(s!=null){this.__BA=s&&this.__BA;}
;}
;var v=this.__BD(t);if(qx.lang.Type.isBoolean(v)){this.__BA=v&&this.__BA;}
;w=v&&w;this.__BH(w);if(qx.lang.Object.isEmpty(this.__By)){this.fireEvent(j);}
;return w;}
,__BB:function(y){if(y.getRequired()){if(this.__BG(y)){var z=!!y.getSelection()[0];}
else {var B=y.getValue();var z=!!B||B===0;}
;y.setValid(z);var C=y.getRequiredInvalidMessage();var A=C?C:this.getRequiredFieldMessage();y.setInvalidMessage(A);return z;}
;return true;}
,__BC:function(D,E){var J=D.item;var I=D.context;var H=D.validator;if(this.__BE(H)){this.__By[J.toHashCode()]=null;H.validate(J,J.getValue(),this,I);return null;}
;var G=null;try{var G=H.call(I||this,E,J);if(G===undefined){G=true;}
;}
catch(e){if(e instanceof qx.core.ValidationError){G=false;if(e.message&&e.message!=qx.type.BaseError.DEFAULTMESSAGE){var F=e.message;}
else {var F=e.getComment();}
;J.setInvalidMessage(F);}
else {throw e;}
;}
;J.setValid(G);D.valid=G;return G;}
,__BD:function(K){var M=this.getValidator();var N=this.getContext()||this;if(M==null){return true;}
;this.setInvalidMessage("");if(this.__BE(M)){this.__By[this.toHashCode()]=null;M.validateForm(K,this,N);return null;}
;try{var O=M.call(N,K,this);if(O===undefined){O=true;}
;}
catch(e){if(e instanceof qx.core.ValidationError){O=false;if(e.message&&e.message!=qx.type.BaseError.DEFAULTMESSAGE){var L=e.message;}
else {var L=e.getComment();}
;this.setInvalidMessage(L);}
else {throw e;}
;}
;return O;}
,__BE:function(P){var Q=false;if(!qx.lang.Type.isFunction(P)){Q=qx.Class.isSubClassOf(P.constructor,qx.ui.form.validation.AsyncValidator);}
;return Q;}
,__BF:function(R){var S=R.constructor;return qx.Class.hasInterface(S,qx.ui.form.IForm);}
,__BG:function(T){var U=T.constructor;return qx.Class.hasInterface(U,qx.ui.core.ISingleSelection);}
,__BH:function(V){var W=this.__Bz;this.__Bz=V;if(W!=V){this.fireDataEvent("changeValid",V,W);}
;}
,getValid:function(){return this.__Bz;}
,isValid:function(){return this.getValid();}
,getInvalidMessages:function(){var X=[];for(var i=0;i<this.__Bx.length;i++){var Y=this.__Bx[i].item;if(!Y.getValid()){X.push(Y.getInvalidMessage());}
;}
;if(this.getInvalidMessage()!=a){X.push(this.getInvalidMessage());}
;return X;}
,getInvalidFormItems:function(){var ba=[];for(var i=0;i<this.__Bx.length;i++){var bb=this.__Bx[i].item;if(!bb.getValid()){ba.push(bb);}
;}
;return ba;}
,reset:function(){for(var i=0;i<this.__Bx.length;i++){var bc=this.__Bx[i];bc.item.setValid(true);}
;this.__Bz=null;}
,setItemValid:function(bd,be){this.__By[bd.toHashCode()]=be;bd.setValid(be);this.__BI();}
,setFormValid:function(bf){this.__By[this.toHashCode()]=bf;this.__BI();}
,__BI:function(){var bh=this.__BA;for(var bi in this.__By){var bg=this.__By[bi];bh=bg&&bh;if(bg==null){return;}
;}
;this.__BH(bh);this.__By={};this.fireEvent("complete");}
},destruct:function(){this.__Bx=null;}
});}
)();
(function(){var a="qx.ui.form.validation.AsyncValidator";qx.Class.define(a,{extend:qx.core.Object,construct:function(b){qx.core.Object.call(this);this.__BJ=b;}
,members:{__BJ:null,__BK:null,__fx:null,__BL:null,validate:function(c,d,e,f){this.__BL=false;this.__BK=c;this.__fx=e;this.__BJ.call(f||this,this,d);}
,validateForm:function(g,h,i){this.__BL=true;this.__fx=h;this.__BJ.call(i,g,this);}
,setValid:function(j,k){if(this.__BL){if(k!==undefined){this.__fx.setInvalidMessage(k);}
;this.__fx.setFormValid(j);}
else {if(k!==undefined){this.__BK.setInvalidMessage(k);}
;this.__fx.setItemValid(this.__BK,j);}
;}
},destruct:function(){this.__fx=this.__BK=null;}
});}
)();
...
(function(){var a="qx.ui.form.Resetter";qx.Class.define(a,{extend:qx.core.Object,construct:function(){qx.core.Object.call(this);this.__nY=[];}
,members:{__nY:null,add:function(b){if(this._supportsValue(b)){var c=b.getValue();}
else if(this.__BG(b)){var c=b.getSelection();}
else if(this.__BQ(b)){var c=b.getSelection().concat();}
else {throw new Error("Item "+b+" not supported for reseting.");}
;this.__nY.push({item:b,init:c});}
,reset:function(){for(var i=0;i<this.__nY.length;i++){var d=this.__nY[i];this.__BO(d.item,d.init);}
;}
,resetItem:function(e){var f;for(var i=0;i<this.__nY.length;i++){var g=this.__nY[i];if(g.item===e){f=g.init;break;}
;}
;if(f===undefined){throw new Error("The given item has not been added.");}
;this.__BO(e,f);}
,__BO:function(h,j){if(this._supportsValue(h)){h.setValue(j);}
else if(this.__BG(h)||this.__BQ(h)){h.setSelection(j);}
;}
,redefine:function(){for(var i=0;i<this.__nY.length;i++){var k=this.__nY[i].item;this.__nY[i].init=this.__BP(k);}
;}
,redefineItem:function(l){var m;for(var i=0;i<this.__nY.length;i++){if(this.__nY[i].item===l){m=this.__nY[i];break;}
;}
;if(m===undefined){throw new Error("The given item has not been added.");}
;m.init=this.__BP(m.item);}
,__BP:function(n){if(this._supportsValue(n)){return n.getValue();}
else if(this.__BG(n)||this.__BQ(n)){return n.getSelection();}
;}
,__BG:function(o){var p=o.constructor;return qx.Class.hasInterface(p,qx.ui.core.ISingleSelection);}
,__BQ:function(q){var r=q.constructor;return qx.Class.hasInterface(r,qx.data.controller.ISelection);}
...
,events:{"changeValue":f},properties:{dateFormat:{check:k,apply:I},placeholder:{check:a,nullable:true,apply:A},appearance:{refine:true,init:B},focusable:{refine:true,init:true},width:{refine:true,init:120}},statics:{__BX:null,__BY:null,getDefaultDateFormatter:function(){var L=qx.locale.Date.getDateFormat(F).toString();if(L==this.__BX){return this.__BY;}
;if(this.__BY){this.__BY.dispose();}
;this.__BY=new qx.util.format.DateFormat(L,qx.locale.Manager.getInstance().getLocale());this.__BX=L;return this.__BY;}
},members:{__Ca:null,_forwardStates:{focused:true,invalid:true},_setDefaultDateFormat:function(){this.setDateFormat(qx.ui.form.DateField.getDefaultDateFormatter());}
,_addLocaleChangeListener:function(){if(qx.core.Environment.get(t)){this.__Ca=qx.locale.Manager.getInstance().addListener(d,function(){this._setDefaultDateFormat();}
,this);}
;}
...
},destruct:function(){if(qx.core.Environment.get(t)){if(this.__Ca){qx.locale.Manager.getInstance().removeListenerById(this.__Ca);}
;}
;}
});}
)();
...
(function(){var k="HH:mm",j="HHmmsszz",h="cldr_date_format_",g="HHmm",f="HHmmss",e="cldr_time_format_",d="HH:mm:ss zz",c="cldr_pm",b="cldr_am",a="qx.locale.Date",J="cldr_date_time_format_",I="HH:mm:ss",H="sat",G="cldr_day_",F="cldr_month_",E="wed",D="fri",C="tue",B="mon",A="sun",r="thu",s="short",p="full",q="long",n="medium",o='_',l="stand-alone",m="narrow",t="abbreviated",u="wide",w='stand-alone',v="qx.debug",y='format',x="format",z="_";qx.Class.define(a,{statics:{__oG:qx.locale.Manager.getInstance(),getAmMarker:function(K){return this.__oG.localize(b,[],K);}
,getPmMarker:function(L){return this.__oG.localize(c,[],L);}
...
;var Q=[A,B,C,E,r,D,H];var R=[];for(var i=0;i<Q.length;i++){var P=G+N+z+length+z+Q[i];R.push(O?this.__oH(N,N===y?w:y,P,M):this.__oG.localize(P,[],M));}
;return R;}
...
;var X=[A,B,C,E,r,D,H];var W=G+U+z+length+z+X[S];return V?this.__oH(U,U===y?w:y,W,T):this.__oG.localize(W,[],T);}
...
;var bd=[];for(var i=0;i<12;i++){var bc=F+ba+z+length+z+(i+1);bd.push(bb?this.__oH(ba,ba===y?w:y,bc,Y):this.__oG.localize(bc,[],Y));}
;return bd;}
...
;var bi=F+bg+z+length+z+(be+1);return bh?this.__oH(bg,bg===y?w:y,bi,bf):this.__oG.localize(bi,[],bf);}
,getDateFormat:function(bj,bk){if(qx.core.Environment.get(v)){qx.core.Assert.assertInArray(bj,[s,n,q,p]);}
;var bl=h+bj;return this.__oG.localize(bl,[],bk);}
,getDateTimeFormat:function(bm,bn,bo){var bq=J+bm;var bp=this.__oG.localize(bq,[],bo);if(bp==bq){bp=bn;}
;return bp;}
,getTimeFormat:function(br,bs){if(qx.core.Environment.get(v)){qx.core.Assert.assertInArray(br,[s,n,q,p]);}
;var bu=e+br;var bt=this.__oG.localize(bu,[],bs);if(bt!=bu){return bt;}
...
else {bJ=this.__oG.getTerritory()||this.__oG.getLanguage();}
;return bJ.toUpperCase();}
,__oH:function(bK,bL,bM,bN){var bP=this.__oG.localize(bM,[],bN);if(bP==bM){var bO=bM.replace(o+bK+o,o+bL+o);return this.__oG.localize(bO,[],bN);}
else {return bP;}
;}
}});}
)();
...
(function(){var cO='W',cN="The constant 'LOGGING_DATE_TIME__format' will be removed (hasn't been used anyway!)",cM="qqqq",cL="h",cK="KK",cJ="Z",cI="GGGGG",cH="LL",cG="LOGGING_DATE_TIME__format",cF="^",bP='4th quarter',bO="SSS",bN="qqq",bM="K",bL='q',bK='G',bJ="a",bI='2nd quarter',bH="dd",bG="qx.util.format.DateFormat",cV='weekDay',cW="ee",cT="q",cU="QQQQ",cR="ww",cS="H",cP='Y+',cQ='wide',cX="(\\d\\d\\d?)",cY="(GMT[\\+\\-]\\d\\d:\\d\\d)",co="_applyLocale",cn="Q",cq="c",cp="weekOfMonth",cs="GGG",cr='-',cu="w",ct="mm",cm='h',cl="S",o='s',p="QQQ",q="G",r="GG",s="kk",t="ss",u="([\\+\\-]\\d\\d\\d\\d)",v='H',w='S',x='1st quarter',dn='Q1',dm='3rd quarter',dl="QQ",dk="HH",ds='k',dr="m",dq="DDD",dp='D',du='K',dt="L",bg="hh",bh="e",be="W",bf="GGGG",bk='Q2',bl="MM",bi="y+",bj='narrow',bc="yyyy-MM-dd HH:mm:ss",bd="short",O='d',N="unkown",Q="\\d?",P="k",K='m',J="String",M='Q3',L="z",I="DD",H='isoUtcDateTime',bq="SS",br="s",bs="M",bt='w',bm="$",bn="?",bo='Q4',bp='month',bu="qq",bv='L',Y='abbreviated',X="Y+",W=":",V='y',U="quoted_literal",T="weekOfYear",S='a',R="day",bb="(-*",ba="locale",bw="GMT",bx="HH:mm:ss",by='y+',bz="HHmmss",bA="long",bB='Y',bC='z',bD='BC',bE='c',bF="d",bT="D",bS='e',bR='B',bQ='',bX='Before Christ',bW='Z',bV="qx.debug",bU=" ",ca="min",bY='M',ch="sec",ci="\\d",cf="+?",cg='E',cd="eeeee",ce="EEEE",cb='Anno Domini',cc="+",cj="eee",ck="ccc",cy="EEEEE",cx="EE",cA="MMMM",cz='Q',cC="eeee",cB="LLLLL",cE="LLLL",cD="ms",cw="(\\d\\d?\\d?)",cv="cccc",dg="dayOfYear",dh='AD',di="ccccc",dj="E",dc="MMM",dd="EEE",de="MMMMM",df="LLL",da="quarter",db='A',n="default",m="0",h="literal",g="wildcard",f="'",e="(\\d?)",d="hour",c="abbreviated",b="wide",a="(\\d\\d*?)",A="narrow",B="-",y="stand-alone",z="",E="format",F="(\\d\\d?)",C="(",D="|",G=")";qx.Class.define(bG,{extend:qx.core.Object,implement:qx.util.format.IFormat,construct:function(dv,dw){qx.core.Object.call(this);if(!dw){this.__jE=qx.locale.Manager.getInstance().getLocale();this.__om=qx.locale.Manager.getInstance().bind(ba,this,ba);}
else {this.__jE=dw;this.setLocale(dw);}
;this.__on=this.__jE;if(dv!=null){this.__oo=dv.toString();if(this.__oo in qx.util.format.DateFormat.ISO_MASKS){if(this.__oo===H){this.__op=true;}
;this.__oo=qx.util.format.DateFormat.ISO_MASKS[this.__oo];}
;}
else {this.__oo=qx.locale.Date.getDateFormat(bA,this.__jE)+bU+qx.locale.Date.getDateTimeFormat(bz,bx,this.__jE);}
;}
,properties:{locale:{apply:co,nullable:true,check:J}},statics:{getDateInstance:function(){var dy=qx.util.format.DateFormat;var dx=qx.locale.Date.getDateFormat(bd)+z;if(dy._dateInstance==null||dy._dateInstance.__oo!=dx){dy._dateInstance=new dy(dx);}
;return dy._dateInstance;}
,getDateTimeInstance:function(){var dA=qx.util.format.DateFormat;var dz=qx.locale.Date.getDateFormat(bA)+bU+qx.locale.Date.getDateTimeFormat(bz,bx);if(dA._dateTimeInstance==null||dA._dateTimeInstance.__oo!=dz){dA._dateTimeInstance=new dA(dz);}
;return dA._dateTimeInstance;}
,ASSUME_YEAR_2000_THRESHOLD:30,LOGGING_DATE_TIME__format:bc,ISO_MASKS:{isoDate:"yyyy-MM-dd",isoTime:"HH:mm:ss",isoDateTime:"yyyy-MM-dd'T'HH:mm:ss",isoUtcDateTime:"yyyy-MM-dd'T'HH:mm:ss'Z'"},AM_MARKER:"am",PM_MARKER:"pm"},members:{__om:null,__jE:null,__on:null,__oo:null,__oq:null,__or:null,__os:null,__op:null,__ot:function(dB,dC){var dD=z+(dB<0?((-1)*dB):dB);while(dD.length<dC){dD=m+dD;}
;return dB<0?B+dD:dD;}
,__ou:function(dE){var dF=new Date(dE.getTime());var dG=dF.getDate();while(dF.getMonth()!=0){dF.setDate(-1);dG+=dF.getDate()+1;}
;return dG;}
,__ov:function(dH){return new Date(dH.getTime()+(3-((dH.getDay()+6)%7))*86400000);}
,__ow:function(dI){var dK=this.__ov(dI);var dL=dK.getFullYear();var dJ=this.__ov(new Date(dL,0,4));return Math.floor(1.5+(dK.getTime()-dJ.getTime())/86400000/7);}
,__ox:function(dM){var dO=this.__ov(dM);var dN=this.__ov(new Date(dM.getFullYear(),dM.getMonth(),4));return Math.floor(1.5+(dO.getTime()-dN.getTime())/86400000/7);}
,__oy:function(dP){var dQ=this.__ov(dP);return dQ.getFullYear();}
,__oz:function(dR){var dS=new Date(dR,2,1);dS.setDate(-1);return dS.getDate()+1===29;}
,__oA:function(dT,dU){var dY=0;var ea=0;if(!dU){dU=1971;}
...
,__oB:function(eb,ec,ed){var ee;switch(ec){case 11:ee=eb-1;if(eb!=this.__oy(new Date(ee,ec,ed))){ee=eb;}
;break;case 0:ee=eb+1;if(eb!=this.__oy(new Date(ee,ec,ed))){ee=eb;}
;break;default:ee=eb;};return ee;}
,_applyLocale:function(ef,eg){this.__jE=ef===null?this.setLocale(this.__on):ef;}
...
;return null;}
;if(this.__op){eh=new Date(eh.getUTCFullYear(),eh.getUTCMonth(),eh.getUTCDate(),eh.getUTCHours(),eh.getUTCMinutes(),eh.getUTCSeconds(),eh.getUTCMilliseconds());}
;var ek=this.__jE;var ep=eh.getFullYear();var el=eh.getMonth();var eo=eh.getDate();var es=eh.getDay();var ev=eh.getHours();var eB=eh.getMinutes();var et=eh.getSeconds();var ez=eh.getMilliseconds();var er=eh.getTimezoneOffset();var ew=er>0?1:-1;var ei=Math.floor(Math.abs(er)/60);var ey=Math.abs(er)%60;this.__oC();var eA=z;for(var i=0;i<this.__os.length;i++){var eq=this.__os[i];if(eq.type==h){eA+=eq.text;}
else {var ej=eq.character;var eu=eq.size;var eC=bn;switch(ej){case V:if(eu==2){eC=this.__ot(ep%100,2);}
else {var em=Math.abs(ep);eC=em+z;if(eu>eC.length){for(var j=eC.length;j<eu;j++){eC=m+eC;}
;}
;if(ep<0){eC=B+eC;}
;}
;break;case bB:eC=this.__oy(eh)+z;var em=eC.replace(cr,bQ);if(eu>eC.length){for(var j=em.length;j<eu;j++){em=m+em;}
;}
;eC=eC.indexOf(B)!=-1?B+em:em;break;case bK:if(eu>=1&&eu<=3){eC=ep>0?dh:bD;}
else if(eu==4){eC=ep>0?cb:bX;}
else if(eu==5){eC=ep>0?db:bR;}
;break;case cz:if(eu==1||eu==2){eC=this.__ot(parseInt(el/4)+1,eu);}
;if(eu==3){eC=cz+(parseInt(el/4)+1);}
;break;case bL:if(eu==1||eu==2){eC=this.__ot(parseInt(el/4)+1,eu);}
;if(eu==3){eC=cz+(parseInt(el/4)+1);}
;break;case dp:eC=this.__ot(this.__ou(eh),eu);break;case O:eC=this.__ot(eo,eu);break;case bt:eC=this.__ot(this.__ow(eh),eu);break;case cO:eC=this.__ox(eh);break;case cg:if(eu>=1&&eu<=3){eC=qx.locale.Date.getDayName(c,es,ek,E,true);}
else if(eu==4){eC=qx.locale.Date.getDayName(b,es,ek,E,true);}
else if(eu==5){eC=qx.locale.Date.getDayName(A,es,ek,E,true);}
;break;case bS:var en=qx.locale.Date.getWeekStart(ek);var ex=1+((es-en>=0)?(es-en):7+(es-en));if(eu>=1&&eu<=2){eC=this.__ot(ex,eu);}
...
;break;case bY:if(eu==1||eu==2){eC=this.__ot(el+1,eu);}
...
;break;case bv:if(eu==1||eu==2){eC=this.__ot(el+1,eu);}
...
;break;case S:eC=(ev<12)?qx.locale.Date.getAmMarker(ek):qx.locale.Date.getPmMarker(ek);break;case v:eC=this.__ot(ev,eu);break;case ds:eC=this.__ot((ev==0)?24:ev,eu);break;case du:eC=this.__ot(ev%12,eu);break;case cm:eC=this.__ot(((ev%12)==0)?12:(ev%12),eu);break;case K:eC=this.__ot(eB,eu);break;case o:eC=this.__ot(et,eu);break;case w:eC=this.__ot(ez,3);if(eu<eC.length){eC=eC.substr(0,eu);}
else {while(eu>eC.length){eC+=m;}
;}
;break;case bC:if(eu>=1&&eu<=4){eC=bw+((ew>0)?B:cc)+this.__ot(Math.abs(ei),2)+W+this.__ot(ey,2);}
;break;case bW:if(eu>=1&&eu<=3){eC=((ew>0)?B:cc)+this.__ot(Math.abs(ei),2)+this.__ot(ey,2);}
else {eC=bw+((ew>0)?B:cc)+this.__ot(Math.abs(ei),2)+W+this.__ot(ey,2);}
;break;};eA+=eC;}
;}
;return eA;}
,parse:function(eD){this.__oD();var eM=this.__oq.regex.exec(eD);if(eM==null){throw new Error("Date string '"+eD+"' does not match the date format: "+this.__oo);}
;var eE={era:1,year:1970,quarter:1,month:0,day:1,dayOfYear:1,hour:0,ispm:false,weekDay:4,weekYear:1970,weekOfMonth:1,weekOfYear:1,min:0,sec:0,ms:0};var eG=1;var eF=false;var eH=false;for(var i=0;i<this.__oq.usedRules.length;i++){var eI=this.__oq.usedRules[i];var eJ=eM[eG];if(eI.field!=null){eE[eI.field]=parseInt(eJ,10);}
else {eI.manipulator(eE,eJ,eI.pattern);}
;if(eI.pattern==X){var eK=false;for(var k=0;k<this.__oq.usedRules.length;k++){if(this.__oq.usedRules[k].pattern==by){eK=true;break;}
;}
;if(!eK){eF=true;}
;}
;if(eI.pattern.indexOf(bT)!=-1){var eO=false;for(var k=0;k<this.__oq.usedRules.length;k++){if(this.__oq.usedRules[k].pattern.indexOf(bF)!=-1){eO=true;break;}
;}
;if(!eO){eH=true;}
;}
;eG+=(eI.groups==null)?1:eI.groups;}
;if(eF){eE.year=this.__oB(eE.weekYear,eE.month,eE.day);}
;if(eH){var eN=this.__oA(eE.dayOfYear,eE.year);eE.month=eN.month;eE.day=eN.day;}
;if(eE.era<0&&(eE.year*eE.era<0)){eE.year=eE.year*eE.era;}
;var eL=new Date(eE.year,eE.month,eE.day,(eE.ispm)?(eE.hour+12):eE.hour,eE.min,eE.sec,eE.ms);if(this.__op){eL=new Date(eL.getUTCFullYear(),eL.getUTCMonth(),eL.getUTCDate(),eL.getUTCHours(),eL.getUTCMinutes(),eL.getUTCSeconds(),eL.getUTCMilliseconds());}
...
;return eL;}
,__oC:function(){if(this.__os!=null){return;}
;this.__os=[];var eT;var eR=0;var eV=z;var eP=this.__oo;var eS=n;var i=0;while(i<eP.length){var eU=eP.charAt(i);switch(eS){case U:if(eU==f){if(i+1>=eP.length){i++;break;}
;var eQ=eP.charAt(i+1);if(eQ==f){eV+=eU;i++;}
else {i++;eS=N;}
;}
else {eV+=eU;i++;}
;break;case g:if(eU==eT){eR++;i++;}
else {this.__os.push({type:g,character:eT,size:eR});eT=null;eR=0;eS=n;}
;break;default:if((eU>=S&&eU<=bC)||(eU>=db&&eU<=bW)){eT=eU;eS=g;}
else if(eU==f){if(i+1>=eP.length){eV+=eU;i++;break;}
;var eQ=eP.charAt(i+1);if(eQ==f){eV+=eU;i++;}
;i++;eS=U;}
else {eS=n;}
;if(eS!=n){if(eV.length>0){this.__os.push({type:h,text:eV});eV=z;}
;}
else {eV+=eU;i++;}
;break;};}
;if(eT!=null){this.__os.push({type:g,character:eT,size:eR});}
else if(eV.length>0){this.__os.push({type:h,text:eV});}
;}
,__oD:function(){if(this.__oq!=null){return;}
;var fa=this.__oo;this.__oF();this.__oC();var fg=[];var fc=cF;for(var eX=0;eX<this.__os.length;eX++){var fh=this.__os[eX];if(fh.type==h){fc+=qx.lang.String.escapeRegexpChars(fh.text);}
else {var eY=fh.character;var fd=fh.size;var fb;for(var fi=0;fi<this.__or.length;fi++){var fe=this.__or[fi];if(this.__oE(fe,eY,fd)){fb=fe;break;}
;}
...
;this.__oq={regex:eW,"usedRules":fg,pattern:fc};}
,__oE:function(fk,fl,fm){if(fl===V&&fk.pattern===by){fk.regex=fk.regexFunc(fm);return true;}
else if(fl===bB&&fk.pattern===cP){fk.regex=fk.regexFunc(fm);return true;}
else {return fl==fk.pattern.charAt(0)&&fm==fk.pattern.length;}
;}
,__oF:function(){var fA=qx.util.format.DateFormat;var fC=qx.lang.String;if(this.__or!=null){return;}
;var fB=this.__or=[];var fw=qx.locale.Date.getAmMarker(this.__jE).toString()||fA.AM_MARKER;var fJ=qx.locale.Date.getPmMarker(this.__jE).toString()||fA.PM_MARKER;var fo=this.__jE;var ft=function(fN,fO){fO=parseInt(fO,10);if(fO>0){if(fO<fA.ASSUME_YEAR_2000_THRESHOLD){fO+=2000;}
...
},destruct:function(){if(this.__om!=null){qx.locale.Manager.getInstance().removeBinding(this.__om);}
;this.__os=this.__oq=this.__or=null;}
...
else {var by=(bv==null)?-1:bv.getDate();for(var i=0;i<6*7;i++){var bx=this.__Cc[i];if(bx.hasState(P)){if(bx.hasState(a)){bx.removeState(a);}
;}
...
;this.__Cb=this.__Cc=this.__Cd=null;}
});}
)();
...
(function(){var m="qx.event.type.Data",k="barRight",j="beforeChangeSelection",h="__rW",g="close-button",f="tabview",d="vertical",c="_applyBarPosition",b="barLeft",a="horizontal",C="qx.ui.tabview.TabView",B="barTop",A="barBottom",z="bottom",y="button",x="qx.debug",w="changeSelection",v="top",u="left",t="close",r="right",s="page",p="firstTab",q="bar",n="lastTab",o="pane";qx.Class.define(C,{extend:qx.ui.core.Widget,implement:qx.ui.core.ISingleSelection,include:[qx.ui.core.MContentPadding],construct:function(D){qx.ui.core.Widget.call(this);this.__rV={top:B,right:k,bottom:A,left:b};this._createChildControl(q);this._createChildControl(o);var E=this.__rW=new qx.ui.form.RadioGroup;E.setWrap(false);E.addListener(w,this._onChangeSelection,this);if(D!=null){this.setBarPosition(D);}
else {this.initBarPosition();}
;}
,events:{"changeSelection":m},properties:{appearance:{refine:true,init:f},barPosition:{check:[u,r,v,z],init:v,apply:c}},members:{__rW:null,_createChildControlImpl:function(F,G){var H;switch(F){case q:H=new qx.ui.container.SlideBar();H.setZIndex(10);this._add(H);break;case o:H=new qx.ui.container.Stack;H.setZIndex(5);this._add(H,{flex:1});break;};return H||qx.ui.core.Widget.prototype._createChildControlImpl.call(this,F);}
...
;}
;var J=I.getButton();var K=this.getChildControl(q);var M=this.getChildControl(o);I.exclude();K.add(J);M.add(I);this.__rW.add(J);I.addState(this.__rV[this.getBarPosition()]);I.addState(n);var L=this.getChildren();if(L[0]==I){I.addState(p);}
...
;var P=N.getButton();var Q=this.getChildControl(q);var S=this.getChildControl(o);N.exclude();Q.addAt(P,O);S.addAt(N,O);this.__rW.add(P);N.addState(this.__rV[this.getBarPosition()]);R=this.getChildren();if(O==R.length-1){N.addState(n);}
...
;}
;W.remove(V);Y.remove(T);this.__rW.remove(V);T.removeState(this.__rV[this.getBarPosition()]);if(T.hasState(p)){T.removeState(p);if(X[0]){X[0].addState(p);}
;}
...
,__rV:null,_applyBarPosition:function(bb,bc){var bd=this.getChildControl(q);var bi=bb==u||bb==r;var bg=bb==r||bb==z;var bh=bi?qx.ui.layout.HBox:qx.ui.layout.VBox;var bk=this._getLayout();if(bk&&bk instanceof bh){}
else {this._setLayout(bk=new bh);}
;bk.setReversed(bg);bd.setOrientation(bi?d:a);var bj=this.getChildren();if(bc){var be=this.__rV[bc];bd.removeState(be);for(var i=0,l=bj.length;i<l;i++){bj[i].removeState(be);}
;}
;if(bb){var bf=this.__rV[bb];bd.addState(bf);for(var i=0,l=bj.length;i<l;i++){bj[i].addState(bf);}
;}
;}
,getSelection:function(){var bl=this.__rW.getSelection();var bm=[];for(var i=0;i<bl.length;i++){bm.push(bl[i].getUserData(s));}
;return bm;}
,setSelection:function(bn){var bo=[];for(var i=0;i<bn.length;i++){bo.push(bn[i].getChildControl(y));}
;this.__rW.setSelection(bo);}
,resetSelection:function(){this.__rW.resetSelection();}
,isSelected:function(bp){var bq=bp.getChildControl(y);return this.__rW.isSelected(bq);}
,isSelectionEmpty:function(){return this.__rW.isSelectionEmpty();}
,getSelectables:function(br){var bs=this.__rW.getSelectables(br);var bt=[];for(var i=0;i<bs.length;i++){bt.push(bs[i].getUserData(s));}
;return bt;}
...
},destruct:function(){this._disposeObjects(h);this.__rV=null;}
});}
)();
