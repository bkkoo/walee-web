...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.data.SearchAsYouType.c7fb61c15cf6.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
(function(){var d="_applySearchString",c="demobrowser.demo.data.filter.SearchAsYouTypeFilter",b="String",a="";qx.Class.define(c,{extend:qx.core.Object,construct:function(e){qx.core.Object.call(this);this.__DG=e;this.filter=qx.lang.Function.bind(function(f){return f.search(this.getSearchString())!=-1;}
,this);this.__vp=null;}
,properties:{searchString:{check:b,apply:d,init:a}},members:{__vp:null,__DG:null,_applySearchString:function(g,h){var i=qx.util.TimerManager.getInstance();if(this.__vp!=null){i.stop(this.__vp);this.__vp=null;}
;this.__vp=i.start(function(){this.__DG.update();this.__vp=null;}
,0,this,null,200);}
,filter:null},destruct:function(){this.__DG=null;}
});}
)();
(function(){var c="qx.util.TimerManager",b="interval",a="singleton";qx.Class.define(c,{extend:qx.core.Object,type:a,statics:{__DH:[],__DI:{},__vp:0},members:{__DJ:false,start:function(d,e,f,g,h){if(!h){h=e||0;}
;var j=(new Date()).getTime()+h;this.self(arguments).__DI[++this.self(arguments).__vp]={callback:d,userData:g||null,expireAt:j,recurTime:e,context:f||this};this.__DK(j,this.self(arguments).__vp);return this.self(arguments).__vp;}
,stop:function(k){var l=this.self(arguments).__DH;var length=l.length;for(var i=0;i<length;i++){if(l[i]==k){l.splice(i,1);break;}
;}
;delete this.self(arguments).__DI[k];if(l.length==0&&this.__DJ){qx.event.Idle.getInstance().removeListener(b,this.__DL,this);this.__DJ=false;}
;}
,__DK:function(m,n){var p=this.self(arguments).__DH;var o=this.self(arguments).__DI;var length=p.length;for(var i=0;i<length;i++){if(o[p[i]].expireAt>m){p.splice(i,0,n);break;}
;}
;if(p.length==length){p.push(n);}
;if(!this.__DJ){qx.event.Idle.getInstance().addListener("interval",this.__DL,this);this.__DJ=true;}
;}
,__DL:function(){var t=(new Date()).getTime();var r=this.self(arguments).__DH;var s=this.self(arguments).__DI;while(r.length>0&&s[r[0]].expireAt<=t){var v=r.shift();var q=s[v];q.callback.call(q.context,q.userData,v);if(q.recurTime&&s[v]){var u=(new Date()).getTime();q.expireAt=u+q.recurTime;this.__DK(q.expireAt,v);}
else {delete s[v];}
;}
;if(r.length==0&&this.__DJ){qx.event.Idle.getInstance().removeListener("interval",this.__DL,this);this.__DJ=false;}
;}
}});}
)();
