...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.bom.HtmlArea.72888a48c339.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
;o=o.concat(n);}
;return qxWeb.$init(o);}
,getAncestors:function(p){return this.__vO(null,p);}
,getAncestorsUntil:function(q,r){return this.__vO(q,r);}
,__vO:function(s,t){var v=[];for(var i=0;i<this.length;i++){var parent=qx.dom.Element.getParentElement(this[i]);while(parent){var u=[parent];if(s&&qx.bom.Selector.matches(s,u).length>0){break;}
...
;return K;}
,getNextAll:function(L){var M=qx.module.Traversing.__vP(this,a,L);return qxWeb.$init(M);}
...
;return T;}
,getPrevAll:function(U){var V=qx.module.Traversing.__vP(this,d,U);return qxWeb.$init(V);}
...
,getSiblings:function(bc){var bd=qx.module.Traversing.__vP(this,c,bc);return qxWeb.$init(bd);}
...
;}
;return this;}
,appendTo:function(parent){parent=qx.module.Manipulating.__vH(parent);for(var i=0,l=parent.length;i<l;i++){for(var j=0,m=this.length;j<m;j++){if(i==0){qx.dom.Element.insertEnd(this[j],parent[i]);}
else {qx.dom.Element.insertEnd(this.eq(j).clone(true)[0],parent[i]);}
;}
;}
;return this;}
,insertBefore:function(p){p=qx.module.Manipulating.__vH(p);for(var i=0,l=p.length;i<l;i++){for(var j=0,m=this.length;j<m;j++){if(i==0){qx.dom.Element.insertBefore(this[j],p[i]);}
else {qx.dom.Element.insertBefore(this.eq(j).clone(true)[0],p[i]);}
;}
;}
;return this;}
,insertAfter:function(q){q=qx.module.Manipulating.__vH(q);for(var i=0,l=q.length;i<l;i++){for(var j=this.length-1;j>=0;j--){if(i==0){qx.dom.Element.insertAfter(this[j],q[i]);}
else {qx.dom.Element.insertAfter(this.eq(j).clone(true)[0],q[i]);}
;}
;}
;return this;}
,__vH:function(r){if(!qx.lang.Type.isArray(r)){var s=qxWeb(r);r=s.length>0?s:[r];}
;return r;}
,wrap:function(t){var t=qx.module.Manipulating.__vI(t);if(t.length==0||!qx.dom.Node.isElement(t[0])){return this;}
;for(var i=0,l=this.length;i<l;i++){var u=t.eq(0).clone(true);qx.dom.Element.insertAfter(u[0],this[i]);var v=qx.module.Manipulating.__vJ(u[0]);qx.dom.Element.insertEnd(this[i],v);}
;return this;}
,__vI:function(w){var x;if(qx.lang.Type.isArray(w)){x=qxWeb(w);}
else {var y=qx.bom.Html.clean([w]);if(y.length>0&&qx.dom.Node.isElement(y[0])){x=qxWeb(y);}
else {x=qxWeb(w);}
;}
;return x;}
,__vJ:function(z){if(z.childNodes.length==0){return z;}
;for(var i=0,l=z.childNodes.length;i<l;i++){if(z.childNodes[i].nodeType===1){return this.__vJ(z.childNodes[i]);}
;}
;return z;}
...
;return L.scrollTop;}
,_animationDescription:{scrollLeft:{duration:700,timing:a,keep:100,keyFrames:{'0':{},'100':{scrollLeft:1}}},scrollTop:{duration:700,timing:a,keep:100,keyFrames:{'0':{},'100':{scrollTop:1}}}},__vK:function(M,N,O){var P=qx.lang.Object.clone(qx.module.Manipulating._animationDescription[M],true);P.keyFrames[100][M]=N;return this.animate(P,O);}
,setScrollLeft:function(Q,R){var Node=qx.dom.Node;if(R&&qx.bom.element&&qx.bom.element.AnimationJs){qx.module.Manipulating.__vK.bind(this,e,Q,R)();}
...
;}
;return this;}
,setScrollTop:function(T,U){var Node=qx.dom.Node;if(U&&qx.bom.element&&qx.bom.element.AnimationJs){qx.module.Manipulating.__vK.bind(this,b,T,U)();}
...
;}
;var bb,bc=[];for(var i=0,l=W.length;i<l;i++){bb=W[i];if(typeof bb===v){bb=this.__vN(bb,X);}
...
;}
,this.__wq);qx.dom.Element.insertBegin(cJ,cI);return cI;}
,__wz:function(){var cQ=qx.dom.Element.create(bF);qx.bom.element.Style.setStyles(cQ,{"marginRight":by,"float":bz});var cS;var cR=qx.dom.Element.create(bk,{title:m});if(qx.core.Environment.get(bB)==bE){qx.bom.element.Attribute.set(cR,bD,bG);}
...
;}
,this.__wq);qx.dom.Element.insertBegin(cR,cQ);return cQ;}
,__wA:function(){var cX=[{bold:{text:co,image:t,action:this.__wq.setBold},italic:{text:bv,image:V,action:this.__wq.setItalic},underline:{text:C,image:bu,action:this.__wq.setUnderline},strikethrough:{text:D,image:cu,action:this.__wq.setStrikeThrough},removeFormat:{text:N,image:h,action:this.__wq.removeFormat}},{alignLeft:{text:cd,image:u,action:this.__wq.setJustifyLeft},alignCenter:{text:Y,image:ce,action:this.__wq.setJustifyCenter},alignRight:{text:bw,image:p,action:this.__wq.setJustifyRight},alignJustify:{text:cm,image:bc,action:this.__wq.setJustifyFull}},{fontFamily:{custom:this.__wy},fontSize:{custom:this.__wz},fontColor:{text:b,image:c,action:this.__ws},textBackgroundColor:{text:bO,image:bS,action:this.__wt}},{indent:{text:bU,image:bo,action:this.__wq.insertIndent},outdent:{text:E,image:n,action:this.__wq.insertOutdent}},{insertImage:{text:o,image:L,action:this.__wu},insertTable:{text:cb,image:f,action:this.__wv},insertLink:{text:bR,image:I,action:this.__ww},insertHTML:{text:P,image:bL,action:this.__wx},insertHR:{text:bK,image:bN,action:this.__wq.insertHorizontalRuler}},{ol:{text:J,image:bp,action:this.__wq.insertOrderedList},ul:{text:A,image:bM,action:this.__wq.insertUnorderedList}},{undo:{text:U,image:g,action:this.__wq.undo},redo:{text:cp,image:ba,action:this.__wq.redo}}];var da,de,cW,dc;for(var i=0,j=cX.length;i<j;i++){de={};de[cs]=i==j-1?bH:by;de[ch]=bz;da=qx.dom.Element.create(bF);qx.bom.element.Style.setStyles(da,de);for(var db in cX[i]){var dd=cX[i][db];if(dd.custom){cW=dd.custom.call(this);}
...
;eS[name]=eT;}
;if(eS.style!==undefined){eM=qx.bom.htmlarea.HtmlArea.__wE(eS.style);delete eS.style;}
...
;}
;for(i=eI.firstChild;i;i=i.nextSibling){eQ.push(qx.bom.htmlarea.HtmlArea.__wF(i,true,eK,eL));}
;if(eJ&&!closed&&eO){eQ.push(dr,eO,bi);}
;break;case 3:eQ.push(eK?eI.data:qx.bom.htmlarea.HtmlArea.__wH(eI.data));break;case 8:eQ.push(dV,eI.data,da);break;};return eQ.join(bV);}
,closingTags:cE,__wG:function(eU){return (qx.bom.htmlarea.HtmlArea.closingTags.indexOf(V+eU.tagName+V)!=-1);}
,__wH:function(s){s=s.replace(/&/ig,u);s=s.replace(/</ig,bR);s=s.replace(/>/ig,du);s=s.replace(/\x22/ig,bz);s=s.replace(/\xA9/ig,dP);return s;}
...
},properties:{contentType:{check:cb,init:bv},messengerMode:{check:cY,init:false},insertParagraphOnLinebreak:{check:cY,init:true},insertLinebreakOnCtrlEnter:{check:cY,init:true},postProcess:{check:dH,nullable:true,init:null},useUndoRedo:{check:cY,init:true},nativeContextMenu:{check:cY,init:false},defaultFontFamily:{check:cb,init:dc},defaultFontSize:{check:bO,init:4}},members:{__lx:null,__eL:false,__wI:false,__wJ:false,__wK:false,__wL:false,__wC:null,__wM:null,__wN:null,__wO:null,__sN:null,__wB:null,__wP:null,__wQ:null,__wR:false,__wS:false,__wT:function(fc){if(qx.dom.Node.isElement(fc)&&qx.dom.Node.isNodeName(fc,dE)){this.__lx=fc;}
;}
,_createAndAddIframe:function(fd){this.__sN=qx.bom.Iframe.create();qx.bom.Iframe.setSource(this.__sN,fd);if((qx.core.Environment.get(bW)==bU)){qx.bom.element.Attribute.set(this.__sN,eq,bI);}
else {qx.bom.element.Style.set(this.__sN,br,ep);}
;qx.bom.element.Style.setStyles(this.__sN,{width:ee,height:ee});qx.dom.Element.insertBegin(this.__sN,this.__lx);}
,_getIframeDocument:function(){return qx.bom.Iframe.getDocument(this.__sN);}
,_getIframeWindow:function(){return qx.bom.Iframe.getWindow(this.__sN);}
,_addIframeLoadListener:function(){qx.event.Registration.addListener(this.__sN,co,this._loaded,this);}
,__wU:function(){this.__wP={"xhtml":{doctype:bF+eo+T+eu+bd+cC,html:et+dT,meta:S,style:qx.core.Environment.select(bW,{"mshtml":bf+dA,"default":dj+er}),contentStyle:dx,body:ek,footer:bh}};}
,__vQ:bV,getIframeObject:function(){return this.__sN;}
,getCommandManager:function(){return this.__wC;}
,setValue:function(fe){if(qx.lang.Type.isString(fe)){this.__vQ=fe;var ff=this._getIframeDocument();if(ff&&ff.body){ff.body.innerHTML=this.__wV(fe);}
;}
;}
,__wV:function(fg){var fj=bV;if((qx.core.Environment.get(bW)==bT)){if(qx.core.Environment.get(en)<=2){fj+=cA;}
;}
;var fk=fg.length==0?by:bV;var fi=qx.core.Environment.get(bW)==bT||qx.core.Environment.get(bW)==bS?ck:bK;var fh=o+U+this.getDefaultFontFamily()+k+cn+fi+dW+this.getDefaultFontSize()+k+fj+fg+fk+p+dN+bu;return fh;}
,getValue:function(){return this.__vQ;}
,getComputedValue:function(fl){return this.getHtml(fl);}
,getCompleteHtml:function(){var fn=this.__wP[this.getContentType()];var fm=fn.html+ea+fn.meta+cM+fn.contentStyle+G+ef;fm+=dh+this.__wW()+dU;fm+=this.getHtml()+bh;return fm;}
,__wW:function(){var fq=[dM,Q,q,ci,cp,dG,P,bq,w,bA,bn,df,dl,di];var fp=qx.bom.element.Style;var fo=this.getContentBody();var ft={};var fs,fu;var fr=qx.core.Environment.get(bW)==bU?2:1;for(var i=0,j=fq.length;i<j;i++){fs=fq[i];fu=fp.get(fo,fs,fr);if(fu!==undefined&&fu!=bV){ft[fs]=fu;}
;}
;return qx.bom.element.Style.compile(ft);}
,getContentDocument:function(){if(this.__eL){return this._getIframeDocument();}
;}
,getContentBody:function(){if(this.__eL){return this._getIframeDocument().body;}
;}
,getContentWindow:function(){if(this.__eL){return this._getIframeWindow();}
;}
...
;}
;return fK;}
,__wX:0,__wY:function(){var fL=this._getIframeDocument();if(!fL){this.__wX++;if(this.__wX>5){this.error('cant load HtmlArea. Document is not available. '+fL);this.fireDataEvent(cV);}
else {if(qx.core.Environment.get(X)){this.error('document not available, try again...');}
;qx.event.Timer.once(function(){this.__wY();}
,this,0);}
;}
else {this.__wX=0;this._onDocumentIsReady();}
;}
,_loaded:function(e){if(this.__wJ){return;}
;if(this.__wI){this.__xa();return;}
;if((qx.core.Environment.get(bW)==bU)){this.__wY();}
else {this._onDocumentIsReady();}
;}
,isReady:function(){return this.__eL;}
...
;this.__wJ=true;if((qx.core.Environment.get(bW)==bU)){this.setEditable(true);}
;this.__xd();if(!(qx.core.Environment.get(bW)==cL)){this.__xe();}
;if(!(qx.core.Environment.get(bW)==bU)){this.setEditable(true);}
;this.__eL=true;this.__wC=fM;fM.setContentDocument(this._getIframeDocument());this.__xj();if((qx.core.Environment.get(bW)==cL)){this.__xe();}
;this.fireEvent(x);}
...
else {fN.body.contentEditable=true;this.__xk(true);}
;}
;}
,"default":(function(){}
)}),invalidateEditor:qx.core.Environment.select(bW,{"gecko":function(){this.__wJ=false;this.__eL=false;this.__wI=true;}
,"default":function(){}
}),__xa:qx.core.Environment.select(bW,{"gecko":function(){this.__xd();this.__xe();this.__wC.setContentDocument(this._getIframeDocument());this.setEditable(true);this.forceEditable();this.__wJ=true;this.__eL=true;this.__wI=false;this.fireEvent(dY);}
,"default":function(){}
}),__xb:function(fO){var fQ=bV;if(!fO){return fQ;}
...
;return fQ;}
,__xc:function(fS){var fT=qx.core.Environment.get(bW)==bT?em:bV;var fU=this.__wP[this.getContentType()];var fV=ea+fU.meta+cM+fT+fU.style+fU.contentStyle+this.__wB+G+ef;var content=fU.body+fS;return fU.html+fV+content+fU.footer;}
,__xd:function(){var fX=this.__wV(this.getValue());if(qx.lang.Type.isString(fX)){var fW=this._getIframeDocument();try{fW.open(bj,true);fW.write(this.__xc(fX));fW.close();}
catch(e){this.error("cant open document on source '"+qx.bom.Iframe.queryCurrentUrl(this.__sN)+"'",e);this.fireDataEvent(cV,e);}
;}
;}
,__xe:function(){this.__xf();this.__xh();this.__xg();}
,__xf:function(){var ga=qx.event.Registration;var fY=this._getIframeDocument();ga.addListener(fY.body,d,this._handleKeyPress,this);ga.addListener(fY.body,cg,this._handleKeyUp,this);ga.addListener(fY.body,cf,this._handleKeyDown,this);}
,__xg:function(){var gd=qx.event.Registration;var gb=this._getIframeDocument();var gc=qx.core.Environment.get(bW)==bS?this._getIframeWindow():gb.body;gd.addListener(gc,E,this._handleFocusEvent,this);gd.addListener(gc,cT,this._handleBlurEvent,this);gd.addListener(gb,ch,this._handleFocusOutEvent,this);}
,__xh:function(){var gg=qx.event.Registration;var gf=this._getIframeDocument();var ge=qx.core.Environment.get(bW)==bU?J:ei;gg.addListener(gf.body,ge,this._handleMouseUpOnBody,this);gg.addListener(gf.documentElement,ge,this._handleMouseUpOnDocument,this);gg.addListener(gf.documentElement,cU,this._handleContextMenuEvent,this);}
,__xi:function(){if(this.__wM==null){this.__wM={execute:function(gh,gi){this.stackedCommands=true;this.commandStack.push({command:gh,value:gi});}
,commandStack:[],stackedCommands:false};}
;this.__wM.stackedCommands=false;return this.__wM;}
,__xj:function(){var gj=this.__wM;if(gj!=null&&gj.stackedCommands){var gk=gj.commandStack;if(gk!=null){for(var i=0,j=gk.length;i<j;i++){this.__wC.execute(gk[i].command,gk[i].value);}
;}
;}
;}
,__xk:function(gl){var gm=this._getIframeDocument();if(this.__wJ&&gm){try{if((qx.core.Environment.get(bW)==bT)){gm.designMode=(gl!==false)?l:dD;}
;gm.designMode=(gl!==false)?dD:l;}
catch(e){}
;}
;}
,setEditable:function(gn){if(this.__wJ){this.__xk(true);if((qx.core.Environment.get(bW)==bT)){try{var go=this._getIframeDocument();go.execCommand(dn,false,true);}
catch(gp){try{var go=this._getIframeDocument();go.execCommand(ba,false,false);}
catch(gq){if(!this.__eL){this.error("Failed to enable rich edit functionality");this.fireDataEvent(cV,gq);}
...
,__xl:false,_handleKeyUp:function(e){var gr=e.getKeyIdentifier().toLowerCase();this.__wN=e;if((qx.core.Environment.get(X))&&qx.core.Environment.get(cF)){this.debug(e.getType()+" | "+gr);}
;if(qx.core.Environment.get(bW)==bU||qx.core.Environment.get(bW)==bS){if(this.__xl){switch(gr){case cO:if(this.getInsertLinebreakOnCtrlEnter()){if((qx.core.Environment.get(bW)==bS)){this.__xm();e.preventDefault();e.stopPropagation();}
else {var gs=this.__xw(this.getSelection());if(gs){gs.collapse(true);gs.pasteHTML(cz);}
;}
;this.__xr();}
;break;case I:this.__xl=false;return;break;};}
;}
else if((qx.core.Environment.get(bW)==bT)){switch(gr){case H:case bl:case cG:case h:case dz:case C:case W:case dB:case eg:this.__wL=(this.getFocusNode()==this.getContentBody().firstChild);break;};}
;}
,__xm:function(){var gt=this.getSelection();var gu=bV;if(gt&&(gt.focusNode.textContent==bV||gt.focusNode.parentElement.tagName==bD)){gu=dJ;}
;this.__wC.execute(cu,gu+qx.bom.htmlarea.HtmlArea.simpleLinebreak);}
...
;if(this.__xl&&(gv==cc||gv==ej||gv==cW||gv==D||gv==dv||gv==eb)){e.preventDefault();e.stopPropagation();}
;if(gv==I){this.__xl=true;}
;}
,"default":function(e){}
}),_handleKeyPress:function(e){var gA=this.getContentDocument();var gw=e.getKeyIdentifier().toLowerCase();var gy=e.isCtrlPressed();var gz=e.isShiftPressed();this.__wN=e;if((qx.core.Environment.get(X))&&qx.core.Environment.get(cF)){this.debug(e.getType()+" | "+gw);}
;if(this.__wR){var gG=!((qx.core.Environment.get(bW)==bU)&&gw==cO)||!((qx.core.Environment.get(bW)==bT)&&gw==cO);if(gG){this.__xr();this.__wR=false;}
;}
...
;e.preventDefault();e.stopPropagation();if((qx.core.Environment.get(bW)==bT)){if(this.__xu()){this.insertHtml(bg);this.__xr();return;}
;this.insertHtml(bb);}
else if((qx.core.Environment.get(bW)==cL)){var gF=this.getSelection();var gB=this.__xw(gF);if(gF&&gB){var gC=gA.createElement(bY);gB.collapse(true);gB.insertNode(gC);gB.collapse(true);gB.selectNode(gC);gF.addRange(gB);gB.collapse(true);}
;}
;this.__xr();}
;if((qx.core.Environment.get(bW)==bU)){if(!this.getInsertParagraphOnLinebreak()){if(this.__wC.execute(cu,qx.bom.htmlarea.HtmlArea.simpleLinebreak)){this.__xr();e.preventDefault();e.stopPropagation();}
;}
;}
else if((qx.core.Environment.get(bW)==bT)){if(this.getInsertParagraphOnLinebreak()&&!gz&&!gy){var gF=this.getSelection();if(gF){var gx=gF.focusNode;if(this.__xu()){this.__xr();return;}
;if(this.__xv()){this.__xr();return;}
;while(!qx.dom.Node.isNodeName(gx,cR)){if(qx.dom.Node.isNodeName(gx,dL)){this.__xr();return;}
;gx=gx.parentNode;}
;}
;this.__wC.insertParagraphOnLinebreak();e.preventDefault();e.stopPropagation();this.__xr();this.__wR=true;}
;}
else if((qx.core.Environment.get(bW)==bS)){if(this.getInsertParagraphOnLinebreak()&&gz){this.__xm();e.preventDefault();e.stopPropagation();this.__xr();}
;}
;break;case cG:if(qx.core.Environment.get(bW)==bT&&qx.core.Environment.get(bX)<1.9&&gz){var gF=this.getSelection();if(gF&&gF.focusNode==gA.body.firstChild){if(this.__wL){if(gF.focusOffset!=0){gF.extend(gF.focusNode,0);}
;}
;}
;}
;this.__xr();break;case cs:if(qx.core.Environment.get(bW)==bT&&qx.core.Environment.get(bX)<1.9){if(gy){var gF=this.getSelection();if(gz){if(gF&&(gF.focusOffset!=0)||(gF.focusNode!=gA.body.firstChild)){gF.extend(gA.body.firstChild,0);}
;}
...
;}
;}
;this.__xr();break;case H:case bl:case h:case dz:case C:case W:case dB:case eg:this.__xr();break;case cW:if(gy){this.__xn(dS,true);}
;break;case dv:case eb:if(gy){this.__xn(bE,true);}
;break;case D:if(gy){this.__xn(dR,true);}
;break;case cc:if(gy&&!gz){this.__xn(dK,true);}
else if(gy&&gz){this.__xn(m,true);}
;break;case ej:if(gy){this.__xn(m,true);}
;break;case t:if(gy){this.selectAll();}
;break;};this.__wN=null;}
,__xn:function(gI,gJ){var gL=null;var gK=qx.bom.htmlarea.HtmlArea.hotkeyInfo;if(gK[gI]){gL=gK[gI].method;}
;if(gL!=null&&this[gL]){this[gL]();if(gJ){this.__wN.preventDefault();this.__wN.stopPropagation();}
;if(this.isSelectionCollapsed()){this.__wR=true;}
;this.__xr();}
;}
,_handleFocusEvent:function(e){this.__wO=null;if(qx.core.Environment.get(bW)==bT||qx.core.Environment.get(bW)==bS){var gM=this.getContentDocument().getElementById(K);if(gM){qx.bom.element.Attribute.reset(gM,cK);}
;}
;this.fireEvent(cq);}
,_handleBlurEvent:function(e){this.__vQ=this.getComputedValue();}
,_handleFocusOutEvent:function(e){this.__xl=false;if(this.__wO==null){this.__wO=this.getSelectedHtml();}
;this.fireEvent(bw);}
,_handleMouseUpOnBody:function(e){if((qx.core.Environment.get(X))&&qx.core.Environment.get(cF)){this.debug("handleMouse "+e.getType());}
;this.__wS=true;this.__xr();}
,_handleMouseUpOnDocument:qx.core.Environment.select(bW,{"mshtml":(function(){}
),"default":function(e){if(!this.__wS){qx.bom.Element.activate(this.getContentBody());}
;this.__wS=false;}
}),_handleContextMenuEvent:function(e){if(!this.getNativeContextMenu()){var gP=e.getViewportLeft();var gO=e.getViewportTop();var gQ=qx.bom.element.Location.getLeft(this.__lx)+gP;var gR=qx.bom.element.Location.getTop(this.__lx)+gO;var gN={x:gQ,y:gR,relX:gP,relY:gO,target:e.getTarget()};e.preventDefault();e.stopPropagation();qx.event.Timer.once(function(){this.fireDataEvent(cU,gN);}
,this,0);}
;}
,isLoaded:function(){return this.__wJ;}
,insertHtml:function(gS){return this.__wC.execute(cu,gS);}
,removeFormat:function(){var gT=this.__wC.execute(v);this.__wC.execute(c,this.getDefaultFontSize());this.__wC.execute(ce,this.getDefaultFontFamily());return gT;}
,setBold:function(){return this.__wC.execute(eh);}
,setItalic:function(){return this.__wC.execute(cS);}
,setUnderline:function(){return this.__wC.execute(cQ);}
,setStrikeThrough:function(){return this.__wC.execute(bk);}
,setFontSize:function(gU){return this.__wC.execute(c,gU);}
,setFontFamily:function(gV){return this.__wC.execute(ce,gV);}
,setTextColor:function(gW){return this.__wC.execute(dF,gW);}
,setTextBackgroundColor:function(gX){return this.__wC.execute(dI,gX);}
,setJustifyLeft:function(){return this.__wC.execute(bt);}
,setJustifyCenter:function(){return this.__wC.execute(bc);}
,setJustifyRight:function(){return this.__wC.execute(cD);}
,setJustifyFull:function(){return this.__wC.execute(bG);}
,insertIndent:function(){return this.__wC.execute(bM);}
,insertOutdent:function(){return this.__wC.execute(cH);}
,insertOrderedList:function(){return this.__wC.execute(es);}
,insertUnorderedList:function(){return this.__wC.execute(bP);}
,insertHorizontalRuler:function(){return this.__wC.execute(dk);}
,insertImage:function(gY){return this.__wC.execute(r,gY);}
,insertHyperLink:function(ha){return this.__wC.execute(bH,ha);}
,removeBackgroundColor:function(){return this.__wC.execute(dC,cx);}
,setBackgroundColor:function(hb){return this.__wC.execute(dC,hb);}
,removeBackgroundImage:function(){return this.__wC.execute(ed);}
,setBackgroundImage:function(hc,hd,he){return this.__wC.execute(ed,[hc,hd,he]);}
,selectAll:function(){return this.__wC.execute(cy);}
,undo:function(){if(this.getUseUndoRedo()){return this.__wC.execute(b);}
else {return true;}
;}
,redo:function(){if(this.getUseUndoRedo()){return this.__wC.execute(dw);}
else {return true;}
;}
...
;return qx.bom.htmlarea.HtmlArea.__wF(hj.body,false,hi,this.getPostProcess());}
...
;this.__xo();}
,"opera":function(){qx.bom.Element.focus(this.getContentWindow());qx.bom.Element.focus(this.getContentBody());this.__xo();}
,"default":function(){qx.bom.Element.focus(this.getContentBody());this.__xo();}
}),__xo:function(){if(!this.__xp()){this.__xq();}
;}
,__xp:qx.core.Environment.select(bW,{"gecko":function(){var hr=this.getContentBody().childNodes;if(hr.length==0){return false;}
...
;}
}),__xq:qx.core.Environment.select(bW,{"gecko|webkit":function(){this.getContentDocument().body.innerHTML=this.__wV(bV);var hu=this.getContentDocument().getElementById(K);qx.bom.element.Attribute.reset(hu,cK);this._selectElement(hu);}
...
;}
}),getContextInformation:function(){return this.__xs();}
,__xr:function(){qx.event.Timer.once(function(e){var hw=this.__xs();this.fireDataEvent(dX,hw);}
,this,200);}
,__xs:function(){if(this._processingExamineCursorContext||this.getEditable()==false){return null;}
;this._processingExamineCursorContext=true;if(!this.__xp()){this.__xq();}
...
else {return this.__xt(hQ);}
;}
,__xt:qx.core.Environment.select(bW,{"mshtml":function(hR){if(!hR){return bV;}
...
;}
}),__xu:qx.core.Environment.select(bW,{"gecko":function(){var hY=this.getSelection();var ia=this.getFocusNode();return hY&&this.isSelectionCollapsed()&&ia!=null&&qx.dom.Node.isText(ia)&&hY.anchorOffset<ia.length;}
,"default":function(){return false;}
}),__xv:qx.core.Environment.select(bW,{"gecko":function(){return qx.dom.Node.isElement(this.getFocusNode());}
,"default":function(){return false;}
}),getRange:function(){return this.__xw(this.getSelection());}
,__xw:qx.core.Environment.select(bW,{"mshtml":function(ib){var ic=this._getIframeDocument();if(ib){try{return ib.createRange();}
...
;}
}),saveRange:qx.core.Environment.select(bW,{"mshtml":function(){this.__wQ=this.getRange();}
,"default":function(){}
}),getSavedRange:qx.core.Environment.select(bW,{"mshtml":function(){return this.__wQ;}
,"default":function(){}
}),resetSavedRange:qx.core.Environment.select(bW,{"mshtml":function(){this.__wQ=null;}
,"default":function(){}
}),getFocusNode:qx.core.Environment.select(bW,{"mshtml":function(){var ii=this.getSelection();var ij;switch(ii.type){case bQ:case cv:ij=this.__xw(ii);if(ij){ij.collapse(false);return ij.parentElement();}
else {return null;}
;break;case R:ij=this.__xw(ii);if(ij){try{ij.collapse(false);}
catch(ik){}
;return ij.item(0);}
else {return null;}
...
;}
;this.__xx.resetSavedRange();return df;}
else {this.error("Command "+dd+" is currently not supported!");}
;}
,__xF:function(){var Node=qx.dom.Node;var dh=this.__xx.getFocusNode();var dk=false;var di=false;if(dh){if(Node.isText(dh)){var dj=qx.dom.Hierarchy.getAncestors(dh);for(var i=0,j=dj.length;i<j;i++){if(Node.isNodeName(dj[i],ce)||qx.bom.htmlarea.HtmlArea.isHeadlineNode(dj[i])){dk=true;break;}
;}
;}
else if(Node.isNodeName(dh,bz)){di=true;}
;}
;return di||!dk;}
,__xG:function(){this.__xH(cu,false,ce);}
,__xH:function(dl,dm,dn){try{var dr=this.__lw;var dp=false;var dq=this.__xx.getRange();this.__lw.body.focus();if((qx.core.Environment.get(cK)==cH)){if(dl!=B){dq.select();if(((dq.text)&&(dq.text.length>0))||((dq.length==1)&&(dq.item(0))&&(dq.item(0).tagName==cV))){dr=dq;}
else {dr=this.__lw;}
;}
;if(((qx.core.Environment.get(cK)==cH))&&(this.__xy[dl])){if(dq.text==cJ){dp=true;}
;}
;}
;var ds=dr.execCommand(dl,dm,dn);if(dp&&dq.text!=cJ){dq.collapse();}
;if((qx.core.Environment.get(cO))&&qx.core.Environment.get(bH)){this.debug("execCommand "+dl+" with value "+dn+" succeded");}
;this.__xD=false;}
catch(dt){if((qx.core.Environment.get(cO))&&qx.core.Environment.get(bH)){this.debug("execCommand "+dl+" with value "+dn+" failed");}
;return false;}
;return ds;}
,__xI:qx.core.Environment.select(cK,{"mshtml":function(){var dv=this.__xx;var du=dv.getSavedRange()!=null?dv.getSavedRange():dv.getRange();return du;}
,"default":function(){return this.__xx.getRange();}
}),__xJ:qx.core.Environment.select(cK,{"mshtml":function(dw,dx){var dA;if(dw==qx.bom.htmlarea.HtmlArea.simpleLinebreak){return this.__xM();}
else {this.__lw.body.focus();var dz=this.__xx.getSelection();var dy=this.__xI();if(dy&&dz&&dz.type!=bE){try{dy.pasteHTML(dw);dy.collapse(false);dy.select();dA=true;}
catch(e){}
;}
else {dA=false;}
;this.__xx.resetSavedRange();return dA;}
;}
,"default":function(dB,dC){this.__lw.body.focus();return this.__lw.execCommand(dC.identifier,false,dB);}
}),insertParagraphOnLinebreak:qx.core.Environment.select(cK,{"gecko":function(){var dK=this.__xW();var dO=this.__xK(dK);var dG=this.__xU(dK);var dL=bw+Date.parse(new Date());var dM=bD+Date.parse(new Date());var dE=s;var dI=bX+dL+cb;dE+=dI;dE+=cT+dM+n+dO+O;dE+=dG+cS;this.__xx.getCommandManager().addUndoStep(cN,ct,this.getCommandObject(cN));this.execute(cN,dE);this.__xL();qx.bom.element.Attribute.reset(this.__lw.getElementById(dL),bI);var dJ=this.__lw.getElementById(dM);if(dJ.previousSibling.innerHTML==dI){var dD=this.__xV();var dH=this.__lw.createElement(S);var dF=this.__lw.createAttribute(p);dF.nodeValue=cJ;dH.setAttributeNode(dF);var dN=this.__lw.createAttribute(cG);dN.nodeValue=cw;dH.setAttributeNode(dN);dJ.previousSibling.appendChild(dD);dJ.previousSibling.appendChild(dH);}
...
;}
;this.__xx.insertHtml(bu+dQ+cA);}
,"default":(function(){}
)}),__xK:qx.core.Environment.select(cK,{"gecko":function(dT){var dY=r;var dX=dT.child;if(dX[ci]){dY+=f+dX[ci]+cl;delete dT.child[ci];}
;if(dX[cf]){dY+=bv+dX[cf]+cl;delete dT.child[cf];}
;var dW={"padding-top":true,"padding-bottom":true,"padding-left":true,"padding-right":true};var dU={"margin-top":true,"margin-bottom":true,"margin-left":true,"margin-right":true};for(var dV in dX){if(dW[dV]||dU[dV]){dY+=dV+bC+dX[dV]+cl;delete dT.child[dV];}
;}
;dY+=cy;return dY;}
,"default":function(){return cJ;}
}),__xL:qx.core.Environment.select(cK,{"gecko":function(){var eh=this.__xx.getSelection();if(!eh||!eh.focusNode){return;}
;var ea=eh.focusNode;var ef=eh.focusNode;while(!qx.dom.Node.isNodeName(ef,ce)){ef=ef.parentNode;}
;var ee=ef.previousSibling.id;var ei=ef.nextSibling?ef.nextSibling.id:null;if(qx.lang.String.startsWith(ee,bD)&&ee==ei){var eg=ef.nextSibling;var ec=this.__xx.getRange();ec.selectNode(eg);eh.addRange(ec);var eb=qx.bom.htmlarea.HtmlArea.EMPTY_DIV;this.__xx.getCommandManager().addUndoStep(cN,eb,this.getCommandObject(cN));this.execute(cN,eb);var ed=this.__xx.getRange();if(ea){while(ea&&ea.firstChild&&qx.dom.Node.isElement(ea.firstChild)){ea=ea.firstChild;}
;}
;ed.selectNode(ea);eh.addRange(ed);ed.collapse(true);}
;}
,"default":(function(){}
)}),__xM:qx.core.Environment.select(cK,{"mshtml":function(){var ej=this.__xx.getRange();if(ej&&!qx.dom.Node.isNodeName(ej.parentElement(),cs)){ej.pasteHTML(qx.bom.htmlarea.HtmlArea.simpleLinebreak);ej.collapse(false);ej.select();return true;}
;return false;}
,"default":function(){return false;}
}),__xN:function(ek,el){var em=(qx.core.Environment.get(cK)==cH)?this.__xx.getRange():this.__lw;return em.execCommand(el.identifier,false,ek);}
,__xO:function(en,eo){if((qx.core.Environment.get(cK)==cH)){var ep=this.__xx.getFocusNode();this.__xP(ep);}
;this.__lw.body.focus();var eq=this.__lw.execCommand(eo.identifier,false,en);if((qx.core.Environment.get(cK)==bP)){var ep=this.__xx.getFocusNode();this.__xP(ep.parentNode);}
;return eq;}
,__xP:function(er){var es=[];var parent=er.parentNode;while(qx.dom.Node.isNodeName(parent,cD)){es.push(parent);parent=parent.parentNode;}
;if(es.length>0){parent.appendChild(er);parent.removeChild(es[es.length-1]);}
;}
,__xQ:qx.core.Environment.select(cK,{"gecko":function(et,eu){if(et.src){this.__lw.execCommand(eu.identifier,false,et.src);delete et.src;var eI=this.__xx.getSelection();if(eI){var eH=eI.anchorNode;var eC=eI.anchorOffset;var ez=eH.childNodes[eC-1];var eF;for(var eB in et){eF=this.__lw.createAttribute(eB);eF.nodeValue=et[eB];ez.setAttributeNode(eF);}
...
;var ey=this.__lw.createDocumentFragment();var eJ;if(eE&&ew!=null){var eA=this.__xR(ew);ey.appendChild(eA.root);eJ=eA.inline;}
else {eJ=this.__lw.createElement(cd);ey.appendChild(eJ);}
;var eG=this.__lw.createTextNode(cJ);eJ.appendChild(eG);var eD=ez.parentNode;if(ez==eD.lastChild){eD.appendChild(ey);}
else {eD.insertBefore(ey,ez.nextSibling);}
;var ev=this.__xx.getRange();ev.selectNodeContents(eJ);}
;return true;}
else {return false;}
;}
,"mshtml":function(eK,eL){var eP=false;var eQ=I;for(var eM in eK){eQ+=eM+bn+eK[eM]+bO;}
;eQ+=z;var eN=this.__xx.getSelection();var eO=this.__xI();if(eN&&eN.type!=bE){try{eO.select();eO.pasteHTML(eQ);eP=true;}
catch(e){}
;}
;this.__xx.resetSavedRange();return eP;}
,"default":function(eR,eS){if(eR.src){var eU=I;for(var eT in eR){eU+=eT+bn+eR[eT]+bO;}
;eU+=z;return this.__lw.execCommand(bJ,false,eU);}
else {return false;}
;}
}),__xR:function(eV){while(eV.firstChild&&eV.firstChild.nodeType==1){eV=eV.firstChild;}
;var fa=this.__xW(eV);var fe,eX,fc;var eW=cJ;var parent=null;var fd=null;var fb=fa.child;while(fb){fd=this.__lw.createElement(fb[cI]?bl:cd);eX=this.__lw.createAttribute(cC);fd.setAttributeNode(eX);for(var eY in fb){if(eY!=bR&&eY!=cI){eW+=eY+bm+fb[eY]+bA;}
else if(eY==cI){fc=this.__lw.createAttribute(bG);fc.nodeValue=fb[eY];fd.setAttributeNode(fc);}
;}
;eX.nodeValue=eW;if(parent!=null){parent.appendChild(fd);}
else {fe=fd;}
;parent=fd;fb=fb.child;eW=cJ;}
;return {root:fe,inline:fd};}
,__xS:qx.core.Environment.select(cK,{"gecko|opera":function(ff,fg){var fm=this.__xx.getSelection();var fj=this.__xx.getRange();if(fm.isCollapsed){var fi=ca+(++this.__xA);var fn=this.__lw.createElement(cx);var fk=this.__lw.createAttribute(bc);var fh=this.__lw.createAttribute(bI);var fl=this.__lw.createTextNode(ff);fh.nodeValue=fi;fn.setAttributeNode(fh);fk.nodeValue=ff;fn.setAttributeNode(fk);fn.appendChild(fl);fj.insertNode(fn);fj.selectNode(fn);fm.collapseToEnd();return true;}
else {return this.__lw.execCommand(fg.identifier,false,ff);}
;}
,"mshtml":function(fo,fp){try{var fr;var fq=this.__xI();var fs=this.__xx;var fq=fs.getSavedRange()!=null?fs.getSavedRange():fs.getRange();if(fq!=null&&fq.text!=cJ){fr=fq.execCommand(fp.identifier,false,fo);}
else {fr=this.__xJ(cp+fo+bQ+fo+bs,fp);}
;this.__xx.resetSavedRange();return fr;}
catch(e){if(qx.core.Environment.get(cO)){this.error("inserthyperlink failed!");}
;return false;}
;}
,"default":function(ft,fu){return this.__lw.execCommand(fu.identifier,false,ft);}
}),__xT:function(fv,fw){var fx=cB;if((qx.core.Environment.get(cK)==cg)){fx+=this.__xU();}
;return this.__xJ(fx,fw);}
,__xU:function(fy){var fC=cJ;var fB=cr;var fA=[];var fF=typeof fy!==bK?fy:this.__xW();var fD=fF.child;var fE=false;if(qx.lang.Object.isEmpty(fD)){return cJ;}
;while(fD){fE=fD[cI]!=null;fC+=fE?cW:fB;for(var fz in fD){fC+=(fz!=bR&&fz!=cI)?fz+bC+fD[fz]+cl:cJ;}
;fC+=fE?L+fD[cI]+bQ:bQ;fA.unshift(fE?H:J);fD=fD.child;}
;if(fA.length==1&&fA[0]==H){fC+=cF;}
;for(var i=0,j=fA.length;i<j;i++){fC+=fA[i];}
;return fC;}
,__xV:function(){var fH=this.__lw.createDocumentFragment();var fM=this.__xW();var parent=fH;var fK=fM.child;var fI;var fL=false;while(fK){fL=fK[cI]!=null;fI=this.__lw.createElement(fL?bl:cd);parent.appendChild(fI);for(var fG in fK){if(fG!=bR&&fG!=cI){qx.bom.element.Style.set(fI,fG,fK[fG]);}
;}
;if(fL){var fJ=this.__lw.createAttribute(bG);fJ.nodeValue=fK[cI];fI.setAttributeNode(fJ);}
;parent=fI;fK=fK.child;}
;return fH;}
,__xW:function(fN){var fP={};var fQ=null;var fR=this.getCurrentStyles(fN);fQ=fP.child={};for(var fO in fR){if(fO!=cM){fQ[fO]=fR[fO];}
;}
...
;return fP;}
,getCurrentStyles:function(fT){if(fT==null){var fV=this.__xx.getSelection();if(!fV||fV.focusNode==null){return {};}
;fT=(fV.focusNode.nodeType==3)?fV.focusNode.parentNode:fV.focusNode;}
;var fW=qx.dom.Hierarchy.getAncestors(fT);var fY=qx.lang.Array.insertBefore(fW,fT,fW[0]);var fU=this.__xX(fY);var fX=this.__xY(fU,fY);return fX;}
,__xX:function(ga){var gd={};var gb,gc;for(var i=0,j=ga.length;i<j;i++){gc=ga[i];for(var k=0,l=gc.style.length;k<l;k++){gb=gc.style[k];if(gb.length>0&&typeof gd[gb]===bK){gd[gb]=gc.style.getPropertyValue(gb);}
;}
;if(gc.tagName.toUpperCase()==co&&gc.size&&gd[cI]===undefined){gd[cI]=gc.size;}
;}
;if(gd[cI]&&gd[bg]){delete gd[bg];}
;return gd;}
,__xY:function(ge,gf){var gk=gf[0];var gi=this.__xx.getContentWindow().getComputedStyle(gk,null);var gg;var gj={};for(var gh in ge){if(gh!=cI){gg=gi.getPropertyValue(gh);}
else {gg=ge[gh];}
;if(gh==bj&&gg==bL){gj[gh]=this.__yb(gf);}
else if(gh==cM){gj[gh]=this.__ya(gf);}
else {gj[gh]=gg;}
;}
;return gj;}
,__ya:function(gl){var go,gq,gm;var gp=[];var gn=this.__xx.getContentWindow();for(var i=0,j=gl.length;i<j;i++){gm=gn.getComputedStyle(gl[i],null);go=gm.getPropertyValue(cM);gq=gm.getPropertyValue(Y);if(go!=cc){gp.push({'text-decoration':go,'color':gq});}
;}
;return gp;}
,__yb:function(gr){var gt,gs,gu;for(var i=0;i<gr.length;i++){gt=gr[i];gs=this.__xx.getContentWindow().getComputedStyle(gt,null);gu=gs.getPropertyValue(bj);if(gu!=bL){return gu;}
;}
;}
,__yc:function(gv,gw){var gI=this.__xx.getSelection();var gD=((qx.core.Environment.get(cK)==cH))?this.__xx.getRange():gD=gI.getRangeAt(0);var gE=[];var gH;var gC=[d,G];var gx,i,j,gF;gF=((qx.core.Environment.get(cK)==cH))?gD.parentElement():gD.commonAncestorContainer;if(gF.tagName==bN){for(var i=0;i<gC.length;i++){gx=gF.getElementsByTagName(gC[i]);for(var j=0;j<gx.length;j++){if(gx[j]){gE.push(gx[j]);}
;}
;}
;}
else if(qx.lang.Array.contains(gC,gF.tagName)){gE.push(gF);}
;if(gE.length>0){for(var i=0;i<gE.length;i++){var gz=gE[i];gH=((qx.core.Environment.get(cK)==cH))?((gz==gF)||(gF.tagName==bN)):gI.containsNode(gz,false);for(j=0;j<gz.childNodes.length;j++){var gB=gz.childNodes[j];if(gH||(gI.focusNode.nodeValue&&qx.dom.Hierarchy.contains(gB,gI.focusNode)&&(gI.focusOffset==gI.focusNode.nodeValue.length))||(qx.dom.Hierarchy.contains(gB,gI.anchorNode)&&(gI.anchorOffset==0))||(gI.containsNode(gB,false))){gB.style.fontSize=(this.__xz[gv]||gv)+by;}
;}
;}
;}
else {var gA=((qx.core.Environment.get(cK)==cH))?gF:gI.focusNode;var gG=qx.dom.Hierarchy.getAncestors(gA);for(i=0;i<gG.length;i++){if(gG[i].tagName==cX){if((((qx.core.Environment.get(cK)==cg))&&((gI.anchorOffset==0)&&(gI.focusNode.nodeValue&&(gI.focusOffset==gI.focusNode.nodeValue.length))&&qx.dom.Hierarchy.contains(gG[i].firstChild,gI.anchorNode)&&qx.dom.Hierarchy.contains(gG[i].lastChild,gI.focusNode)))||(((qx.core.Environment.get(cK)==cH))&&(gD.htmlText==gG[i].innerHTML))){gG[i].style.fontSize=(this.__xz[gv]||gv)+by;}
;break;}
;}
;}
;if((qx.core.Environment.get(cK)==cH)){this.__lw.body.focus();this.__xx.getRange().select();return this.__lw.execCommand(cj,false,gv);}
...
;}
;}
;return this.__lw.execCommand(cj,false,gv);}
,__yd:qx.core.Environment.select(cK,{"mshtml":function(gJ,gK){this.__lw.body.focus();return this.__lw.execCommand(ck,false,gJ);}
,"gecko|opera":function(gL,gM){this.__lw.body.focus();return this.__lw.execCommand(R,false,gL);}
,"webkit":function(gN,gO){var gP=this.__xx.getSelection();var gQ=this.__xx.getRange();if(!gP||!gP.isCollapsed){this.__lw.body.focus();this.__lw.execCommand(ck,false,gN);if(gP){gP.collapseToEnd();}
;return true;}
...
;}
;gQ.setStart(gP.anchorNode,gP.anchorNode.nodeValue.charAt(gT)==bB?gT+1:gT);gQ.setEnd(gP.anchorNode,gS);gP.addRange(gQ);this.__lw.body.focus();this.__lw.execCommand(ck,false,gN);gP.collapseToEnd();return true;}
;}
}),__ye:function(gU,gV){gU=gU!=null&&typeof gU==bi?gU:bL;qx.bom.element.Style.set(this.__lw.body,C,gU);return true;}
,__yf:function(gW,gX){var gY,hb,ha;var hc=qx.bom.htmlarea.manager.Command;if(gW==null){gY=null;}
else {gY=gW[0];hb=gW[1];ha=gW[2];}
;if(gY==null||typeof gY!=bi){qx.bom.element.Style.set(this.__lw.body,bF,cJ);qx.bom.element.Style.set(this.__lw.body,bk,cJ);qx.bom.element.Style.set(this.__lw.body,be,cJ);return true;}
else {if(gY.search(/^url.*\(/)==-1){gY=m+gY+W;}
;}
;if(hb!=null&&!qx.lang.String.contains(hc.__xC,hb)){if(qx.core.Environment.get(cO)){this.error("The value '"+hb+"' is not allowed for parameter 'repeat'. Possible values are '"+hc.__xC+"'");}
;return false;}
else {hb=cq;}
;if(ha!=null){if(qx.lang.Type.isString(ha)&&!qx.lang.String.contains(hc.__xB,bh+ha+bh)){if(qx.core.Environment.get(cO)){this.error("The value '"+ha+"' is not allowed for parameter 'position'. Possible values are '"+hc.__xB+"'");}
;return false;}
...
;qx.bom.element.Style.set(this.__lw.body,bF,gY);qx.bom.element.Style.set(this.__lw.body,bk,hb);qx.bom.element.Style.set(this.__lw.body,be,ha);return true;}
,__yg:qx.core.Environment.select(cK,{"mshtml":function(hd,he){var hf=this.__lw.body.createTextRange();hf.select();return true;}
,"default":function(hg,hh){return this.__xH(hh.identifier,false,hg);}
}),__yh:function(){return this.__xx.getSelectedText();}
,__yi:function(){return this.__xx.getSelectedHtml();}
,__yj:function(hi,hj){var hn=this.__xx.getFocusNode();if(hn.textContent==cJ){var hl=qx.dom.Hierarchy.getAncestors(hn);hl.unshift(hn);var Node=qx.dom.Node;var hk=qx.bom.element.Style;var hm=hl.shift();while(hl.length>0){if(Node.getName(hm)==ce||Node.getName(hm)==bz){break;}
;if(hk.get(hm,hi,hk.LOCAL_MODE)==hj){hk.reset(hm,hi);return true;}
;hm=hl.shift();}
;}
;return false;}
,__yk:qx.core.Environment.select(cK,{"gecko":function(ho,hp){if(this.__yj(P,bo)){return true;}
else {return this.__xH(hp.identifier,false,ho);}
;}
,"default":function(hq,hr){return this.__xH(hr.identifier,false,hq);}
}),__yl:qx.core.Environment.select(cK,{"gecko":function(hs,ht){if(this.__yj(bS,bW)){return true;}
else {return this.__xH(ht.identifier,false,hs);}
;}
,"default":function(hu,hv){return this.__xH(hv.identifier,false,hu);}
}),__ym:qx.core.Environment.select(cK,{"gecko":function(hw,hx){if(this.__yj(bM,a)){return true;}
else {return this.__xH(hx.identifier,false,hw);}
;}
,"default":function(hy,hz){return this.__xH(hz.identifier,false,hy);}
}),__yn:qx.core.Environment.select(cK,{"gecko":function(hA,hB){if(this.__yj(bM,V)){return true;}
else {return this.__xH(hB.identifier,false,hA);}
;}
,"default":function(hC,hD){return this.__xH(hD.identifier,false,hC);}
})},destruct:function(){this.__lw=this.__xx=this._commands=null;this.__xy=this.__xz=null;}
});}
)();
(function(){var m="undo",l="pageup",k="b",h="control",g="shift",f="end",d="right",c="qx_link",b="insertunorderedlist",a="Undo",bi="pagedown",bh="k",bg="home",bf="a",be="insertorderedlist",bd="y",bc="justifyright",bb="__wC",ba="redo",Y="insertParagraph",t="qx.bom.htmlarea.manager.UndoRedo",u="left",r="i",s="u",p="enter",q="Redo",n="insertimage",o="z",x="undoRedoState",y="down",G="up",E="body",O="backgroundColor",J="mousedown",U="table",S="backgroundImage",A="backgroundRepeat",X="p",W="backgroundPosition",V="td",z="img",C="inserthyperlink",D="mouseup",F="keypress",H="qx.bom.htmlarea.HtmlArea.debug",K="backgroundimage",P="qx.debug",T="backgroundcolor",v="webkit",w="Content",B="Internal",N="inserthtml",M="gecko",L="Custom",R="Command",Q="mshtml",I="engine.name";qx.Class.define(t,{extend:qx.core.Object,construct:function(bj,bk){qx.core.Object.call(this);this.__wC=bj;this.__xx=bk;this.__yo=[];this.__yp=[];this._commands=null;this.__lw=null;this.__yq={};this.__yr={command:true,content:true,custom:true};this.__xE();this.__ys=qx.lang.Function.bind(this._handleKeyPress,this);this.__yt=qx.lang.Function.bind(this._handleMouseUp,this);if((qx.core.Environment.get(I)==Q)){this.__yu=qx.lang.Function.bind(this._handleMouseDown,this);}
;}
,members:{__yv:false,__yw:false,__yx:false,__yr:null,__yq:null,__wC:null,__lw:null,__yo:null,__yp:null,__xx:null,__ys:null,__yt:null,__yu:null,__yy:null,setContentDocument:function(bl){this.__lw=bl;this.__wC.setContentDocument(bl);qx.event.Registration.addListener(bl.body,F,this.__ys,this);qx.event.Registration.addListener(bl.body,D,this.__yt,this);if((qx.core.Environment.get(I)==Q)){qx.event.Registration.addListener(bl.body,J,this.__yu,this,true);}
;}
,insertParagraphOnLinebreak:function(){return this.__wC.insertParagraphOnLinebreak();}
,execute:function(bm,bn){var bo;bm=bm.toLowerCase();if(this._commands[bm]){if(this._commands[bm].passthrough){bo=this.__wC.execute(bm,bn);}
else {bo=this[bm].call(this);}
;}
else {if((qx.core.Environment.get(I)==Q)||(qx.core.Environment.get(I)==v)){this.__yP(bm,bn,this.__wC.getCommandObject(bm));bo=this.__wC.execute(bm,bn);if(!bo){this.__yo.pop();}
;}
else {bo=this.__wC.execute(bm,bn);if(bo){this.__yP(bm,bn,this.__wC.getCommandObject(bm));}
;}
;if(bm==m&&this.__yo.length==0){this.__yw=false;this.__yX();}
else if(bm==ba&&this.__yp.length==0){this.__yv=false;this.__yX();}
;}
;this.__yx=false;return bo;}
,addUndoStep:function(bp,bq,br){this.__yP(bp,bq,br);}
,registerHandler:function(bs,bt,bu,bv){this.__yq[bs]={undo:bt,redo:bu,context:bv};}
,isUndoPossible:function(){return this.__yw;}
,undo:function(){var by;if(this.__yx){this.__yR();}
;if(this.__yo.length>0){var bx=this.__yo.pop();if(this.__yr[bx.actionType.toLowerCase()]){switch(bx.actionType){case R:by=this.__yA(bx);break;case w:by=this.__yE(bx);break;case B:by=this.__yD(bx);break;case L:by=this.__yz(bx);break;};}
else if(this.__yq[bx.actionType]){var bw=this.__yq[bx.actionType];by=bw.undo.call(bw.context?bw.context:this,bx);this.__yT(bx);}
else {this.error("actionType "+bx.actionType+" is not managed! Please provide a handler method!");}
;this.__yv=true;this.__yX();return by;}
;}
,__yz:qx.core.Environment.select(I,{"mshtml|webkit":function(bz){var bA=this.__lw.body.innerHTML;var bC=bz.content;this.__lw.body.innerHTML=bC;var bB=bz;bB.content=bA;this.__yT(bB);return true;}
,"default":function(bD){var bH=bD;var bG=qx.bom.element.Style;switch(bD.command){case T:bH.parameter=[bG.get(this.__lw.body,O)];break;case K:bH.parameter=[bG.get(this.__lw.body,S),bG.get(this.__lw.body,A),bG.get(this.__lw.body,W)];break;};this.__yT(bH);if(bD.command==C){if((qx.core.Environment.get(I)==M)){var bF=c+this.__wC.__hyperLinkId;var bE=this.__lw.getElementById(bF);if(bE){bE.parentNode.removeChild(bE);return true;}
else {return false;}
;}
;}
else {return this.__wC.execute(bD.command,bD.value);}
;}
}),__yA:qx.core.Environment.select(I,{"mshtml|webkit":function(bI){}
,"default":function(bJ){this.__yT(bJ);if((qx.core.Environment.get(I)==M)){if(bJ.command==N&&bJ.value==qx.bom.htmlarea.HtmlArea.EMPTY_DIV&&this.__yB(N,Y)){this.__yC();}
;}
;return this.__yF();}
}),__yB:function(bK,bL){if(this.__yo.length>0){var bM=this.__yo[this.__yo.length-1];return (bM.command==bK&&bM.value==bL);}
;return false;}
,__yC:function(){this.__yF();if(this.__yo.length>0){var bN=this.__yo.pop();this.__yT(bN);}
;}
,__yD:function(bO){this.__yT(bO);return this.__yF();}
,__yE:qx.core.Environment.select(I,{"gecko":function(bP){this.__yT(bP);try{return this.__yF();}
catch(bQ){if(qx.core.Environment.get(P)){this.error("execCommand failed! Details: "+bQ);}
;}
;}
,"mshtml|webkit":function(bR){}
,"default":function(bS){this.__yT(bS);return this.__yF();}
}),__yF:function(){try{return this.__lw.execCommand(a,false,null);}
catch(e){return false;}
;}
,isRedoPossible:function(){return this.__yv;}
,redo:function(){if(this.__yv){var bU;if(this.__yp.length>0){var bV=this.__yp.pop();if(this.__yr[bV.actionType.toLowerCase()]){switch(bV.actionType){case R:bU=this.__yH(bV);break;case w:bU=this.__yN(bV);break;case B:bU=this.__yM(bV);break;case L:bU=this.__yG(bV);break;};}
else if(this.__yq[bV.actionType]){var bT=this.__yq[bV.actionType];bU=bT.redo.call(bT.context?bT.context:this,bV);this.__yS(bV);}
else {this.error("actionType "+bV.actionType+" is not managed! Please provide a handler method!");}
;this.__yw=true;this.__yX();}
;return bU;}
;}
,__yG:qx.core.Environment.select(I,{"mshtml|webkit":function(bW){var bX=this.__lw.body.innerHTML;var ca=bW.content;this.__lw.body.innerHTML=ca;var bY=bW;bY.content=bX;this.__yS(bY);return true;}
,"default":function(cb){this.__yS(cb);return this.__yO();}
}),__yH:qx.core.Environment.select(I,{"mshtml|webkit":function(cc){}
,"default":function(cd){this.__yS(cd);var ce=this.__yO();if((qx.core.Environment.get(I)==M)){if(this.__yI(N,qx.bom.htmlarea.HtmlArea.EMPTY_DIV)){var cf=this.__yJ();this.__yK();if(cf!=null){this.__yL(cf);}
;}
;}
;return ce;}
}),__yI:function(cg,ch){if(this.__yp.length>0){var ci=this.__yp[this.__yp.length-1];return (ci.command==cg&&ci.value==ch);}
;return false;}
,__yJ:function(){if(this.__xx==null){return null;}
;var cj=this.__xx.getSelection();var ck=cj?cj.focusNode:null;if(ck==null){return null;}
...
;}
,__yK:function(){var cm=this.__yp.pop();this.__yS(cm);this.__yO();}
,__yL:qx.core.Environment.select(I,{"gecko":function(cn){if(cn==this.__xx.getContentBody().lastChild){return;}
;var cq=cn.firstChild;while(cq.firstChild){cq=cq.firstChild;}
;var co=this.__xx.getSelection();var cp=this.__xx.getRange();if(co&&cp){cp.selectNode(cq);co.addRange(cp);cp.collapse(true);}
;}
,"default":(function(){}
)}),__yM:function(cr){this.__yS(cr);return this.__yO();}
,__yN:qx.core.Environment.select(I,{"mshtml|webkit":function(cs){}
,"default":function(ct){this.__yS(ct);return this.__yO();}
}),__yO:function(){try{return this.__lw.execCommand(q,false,null);}
catch(e){return false;}
;}
,__xE:function(){this._commands={undo:{passthrough:false},redo:{passthrough:false}};this.__wC.getCommandObject(T).customUndo=true;this.__wC.getCommandObject(K).customUndo=true;if((qx.core.Environment.get(I)==M)){}
;}
,__yP:qx.core.Environment.select(I,{"mshtml|webkit":function(cu,cv,cw){var cx=this.getUndoRedoObject();cx.commandObject=cw;cx.command=cu;cx.value=cv;cx.actionType=L;cx.content=this.__lw.body.innerHTML;this.__yQ(cx);}
,"default":function(cy,cz,cA){if(this.__xx==null){return;}
;var cC=this.getUndoRedoObject();cC.commandObject=cA;cC.command=cy;cC.value=cz;cC.actionType=L;var cB=this.__xx.getSelection();if(cA.customUndo){var cD=[];switch(cy){case T:cD.push(qx.bom.element.Style.get(this.__lw.body,O));break;case K:cD.push(qx.bom.element.Style.get(this.__lw.body,S),qx.bom.element.Style.get(this.__lw.body,A),qx.bom.element.Style.get(this.__lw.body,W));break;case C:if(cB&&!cB.isCollapsed){cC.actionType=R;}
...
;}
;this.__yQ(cC);}
}),__yQ:function(cE){if(this.__yx){this.__yR();}
;this.__yS(cE);this.__yv=false;this.__yp=[];this.__yX();}
,__yR:function(){var cF=this.__yo[this.__yo.length-1];if(cF==null||cF.actionType!=w){var cG=this.getUndoRedoObject();cG.actionType=w;if((qx.core.Environment.get(I)==Q)||(qx.core.Environment.get(I)==v)){cG.content=this.__yy;cG.actionType=L;this.__yy=null;}
;this.__yS(cG);this.__yx=false;}
;}
...
,__yS:function(cH){if((qx.core.Environment.get(P))&&qx.core.Environment.get(H)){this.debug("ADD TO UNDO STACK");this.debug(cH.actionType+" "+cH.command+" "+cH.value);}
;this.__yo.push(cH);}
,__yT:function(cI){if((qx.core.Environment.get(P))&&qx.core.Environment.get(H)){this.debug("ADD TO REDO STACK");this.debug(cI.actionType+" "+cI.command+" "+cI.value);}
;this.__yp.push(cI);}
,_handleKeyPress:function(e){var cJ=e.getKeyIdentifier().toLowerCase();var cK=e.isCtrlPressed();switch(cJ){case h:case g:case u:case d:case G:case y:case l:case bi:case bg:case f:case p:break;case bf:case k:case r:case s:case bh:case bd:case o:if(!cK){this.__yU();}
;break;default:this.__yv=false;this.__yp=[];this.__yU();};}
,__yU:function(){if(!this.__yx){this.__yx=true;this.__yw=true;if((qx.core.Environment.get(I)==Q)||(qx.core.Environment.get(I)==v)){this.__yy=this.__lw.body.innerHTML;}
;this.__yX();}
;}
,__yV:null,_handleMouseDown:qx.core.Environment.select(I,{"mshtml":function(e){var cL=e.getOriginalTarget();if(qx.dom.Node.isElement(cL)&&(qx.dom.Node.isNodeName(cL,z)||qx.dom.Node.isNodeName(cL,U))){this.__yV={node:cL,content:cL.outerHTML};}
else {this.__yV=null;}
;}
,"default":function(e){return true;}
}),_handleMouseUp:qx.core.Environment.select(I,{"gecko":function(e){if(this.__xx==null){return;}
;var cN=this.__xx.getSelection();if(!cN){this.__yV=null;return;}
;var cM=cN.anchorNode;var cO=cM.childNodes[cN.anchorOffset];if(qx.dom.Node.isNodeName(cO,z)){if(this.__yV==null){this.__yV=cO.cloneNode(true);}
else {if(this.__yV.style.width!=cO.style.width||this.__yV.style.height!=cO.style.height){this.__yW();this.__yV=cO.cloneNode(true);}
;}
;}
...
;if(this.__yV==null){this.__yV=cP.cloneNode(true);}
else {qx.event.Timer.once(function(){if(cP.style.width!=this.__yV.style.width||cP.style.height!=this.__yV.style.height||cP.innerHTML!=this.__yV.innerHTML){this.__yW();this.__yV=cP.cloneNode(true);}
;}
,this,0);}
;}
else {this.__yV=null;}
;}
,"default":function(e){var cQ=qx.bom.Event.getTarget(e);if(this.__yV!=null){if(cQ.nodeType==1){if(cQ==this.__yV.node){if(cQ.outerHTML!=this.__yV.content){this.__yV.content=cQ.outerHTML;this.__yW();}
;}
else {for(var i=0,j=cQ.childNodes.length;i<j;i++){if(cQ.childNodes[i]==this.__yV.node){if(cQ.childNodes[i].outerHTML!=this.__yV.content){this.__yV.content=cQ.childNodes[i].outerHTML;this.__yW();}
;}
;}
;}
;}
...
;}
;}
}),__yW:qx.core.Environment.select(I,{"mshtml|webkit":function(){this.__yP(B,null,null);}
,"default":function(){var cR=this.getUndoRedoObject();cR.actionType=B;this.__yS(cR);}
}),__yX:function(){qx.event.Timer.once(function(e){if(this.__xx!=null){var cS={undo:this.isUndoPossible()?0:-1,redo:this.isRedoPossible()?0:-1};this.__xx.fireDataEvent(x,cS);}
;}
,this,200);}
},destruct:function(){try{qx.event.Registration.removeListener(this.__lw.body,F,this.__ys);qx.event.Registration.removeListener(this.__lw,D,this.__yt);if((qx.core.Environment.get(I)==Q)){qx.event.Registration.removeListener(this.__lw,J,this.__yu);}
;}
catch(e){}
;this._disposeObjects(bb);this.__xx=this.__yo=this.__yp=this._commands=this.__lw=null;this.__yr=this.__yq=null;}
});}
)();
