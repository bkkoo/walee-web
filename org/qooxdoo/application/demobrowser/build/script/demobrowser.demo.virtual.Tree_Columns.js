...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.virtual.Tree_Columns.b5940c29c18e.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
(function(){var l="changeModel",k="_applyLabelOptions",j="_applyOpenMode",h="qx.core.Object",g="Space",f="Left",d="Integer",c="_applyModel",b="_applyShowTopLevelOpenCloseIcons",a="_applyLabelPath",N="changeOpenMode",M="qx.debug",L="changeDelegate",K="virtual-tree",J="_applyChildProperty",I="_applyIconPath",H="_applyDelegate",G="_applyHideRoot",F="close",E="_applyRowHeight",s="Right",t="Enter",q="qx.ui.tree.VirtualTree",r="keypress",o="none",p="_applyShowLeafs",m="_applyIconOptions",n=".",u="one",v="qx.event.type.Data",y="updated",x="Boolean",A="click",z="String",C="changeBubble",B="dblclick",w="cellDblclick",D="cellClick";qx.Class.define(q,{extend:qx.ui.virtual.core.Scroller,implement:[qx.ui.tree.core.IVirtualTree,qx.data.controller.ISelection],include:[qx.ui.virtual.selection.MModel,qx.ui.core.MContentPadding],construct:function(O,P,Q){qx.ui.virtual.core.Scroller.call(this,0,1,20,100);this._init();if(P!=null){this.setLabelPath(P);}
;if(Q!=null){this.setChildProperty(Q);}
;if(O!=null){this.initModel(O);}
;this.initItemHeight();this.initOpenMode();this.addListener(r,this._onKeyPress,this);}
,events:{open:v,close:v},properties:{appearance:{refine:true,init:K},focusable:{refine:true,init:true},width:{refine:true,init:100},height:{refine:true,init:200},itemHeight:{check:d,init:25,apply:E,themeable:true},openMode:{check:[A,B,o],init:B,apply:j,event:N,themeable:true},hideRoot:{check:x,init:false,apply:G},showTopLevelOpenCloseIcons:{check:x,init:false,apply:b},showLeafs:{check:x,init:true,apply:p},childProperty:{check:z,apply:J,nullable:true},labelPath:{check:z,apply:a,nullable:true},iconPath:{check:z,apply:I,nullable:true},labelOptions:{apply:k,nullable:true},iconOptions:{apply:m,nullable:true},model:{check:h,apply:c,event:l,nullable:true,deferredInit:true},delegate:{event:L,apply:H,init:null,nullable:true}},members:{_provider:null,_layer:null,__Ap:null,__IJ:null,__IK:null,__gg:null,__IL:0,__IM:null,syncWidget:function(R){var V=this._layer.getFirstRow();var S=this._layer.getRowSizes().length;for(var W=V;W<V+S;W++){var T=this._layer.getRenderedCellWidget(W,0);if(T!=null){this.__IL=Math.max(this.__IL,T.getSizeHint().width);}
;}
;var U=this.getPane().getInnerSize().width;this.getPane().getColumnConfig().setItemSize(0,Math.max(this.__IL,U));}
,openNode:function(X){this.__IP(X);this.buildLookupTable();}
,refresh:function(){this.buildLookupTable();}
,openNodeAndParents:function(Y){this.__IQ(this.getModel(),Y);this.buildLookupTable();}
,closeNode:function(ba){if(qx.lang.Array.contains(this.__IJ,ba)){qx.lang.Array.remove(this.__IJ,ba);this.fireDataEvent(F,ba);this.buildLookupTable();}
;}
,isNodeOpen:function(bb){return qx.lang.Array.contains(this.__IJ,bb);}
,_init:function(){this.__Ap=new qx.data.Array();this.__IJ=[];this.__IK=[];this._initLayer();}
,_initLayer:function(){this._provider=new qx.ui.tree.provider.WidgetProvider(this);this._layer=this._provider.createLayer();this._layer.addListener(y,this._onUpdated,this);this.getPane().addLayer(this._layer);}
,getLookupTable:function(){return this.__Ap;}
,_reverseLookup:function(bc){return bc;}
,_getDataFromRow:function(bd){return this.__Ap.getItem(bd);}
,_getSelectables:function(){return this.__Ap;}
,getOpenNodes:function(){return this.__IJ;}
,isNode:function(be){return qx.Class.hasProperty(be.constructor,this.getChildProperty());}
,getLevel:function(bf){return this.__IK[bf];}
,hasChildren:function(bg){var bh=bg.get(this.getChildProperty());if(bh==null){return false;}
;if(this.isShowLeafs()){return bh.length>0;}
else {for(var i=0;i<bh.getLength();i++){var bi=bh.getItem(i);if(this.isNode(bi)){return true;}
;}
;}
;return false;}
...
,_applyOpenMode:function(bl,bm){var bn=this.getPane();if(bl===B){bn.addListener(w,this._onOpen,this);}
else if(bl===A){bn.addListener(D,this._onOpen,this);}
;if(bm===B){bn.removeListener(w,this._onOpen,this);}
else if(bm===A){bn.removeListener(D,this._onOpen,this);}
;}
...
,_applyModel:function(bE,bF){this.__IJ=[];if(bE!=null){if(qx.core.Environment.get(M)){if(!qx.Class.hasMixin(bE.constructor,qx.data.marshal.MEventBubbling)){this.warn("The model item doesn't support the Mixin 'qx.data."+"marshal.MEventBubbling'. Therefore the tree can not update "+"the view automatically on model changes.");}
;}
;bE.addListener(C,this._onChangeBubble,this);this.__IP(bE);}
;if(bF!=null){bF.removeListener(C,this._onChangeBubble,this);}
;this.__IN();}
,_applyDelegate:function(bG,bH){this._provider.setDelegate(bG);this.buildLookupTable();}
,_onChangeBubble:function(event){var bJ=event.getData().name;var bI=bJ.lastIndexOf(n);if(bI!=-1){bJ=bJ.substr(bI+1,bJ.length);}
;if(qx.lang.String.startsWith(bJ,this.getChildProperty())){this.__IN();}
;}
,_onUpdated:function(event){if(this.__gg==null){this.__gg=new qx.util.DeferredCall(function(){qx.ui.core.queue.Widget.add(this);}
,this);}
;this.__gg.schedule();}
,_onOpen:function(event){var bL=event.getRow();var bK=this.__Ap.getItem(bL);if(this.isNode(bK)){if(this.isNodeOpen(bK)){this.closeNode(bK);}
else {this.openNode(bK);}
;}
;}
,_onKeyPress:function(e){var bO=this.getSelection();if(bO.getLength()>0){var bM=bO.getItem(0);var bN=this.isNode(bM);switch(e.getKeyIdentifier()){case f:if(bN&&this.isNodeOpen(bM)){this.closeNode(bM);}
else {var parent=this.getParent(bM);if(parent!=null){bO.splice(0,1,parent);}
;}
;break;case s:if(bN&&!this.isNodeOpen(bM)){this.openNode(bM);}
else {if(bN){var bP=bM.get(this.getChildProperty());if(bP!=null&&bP.getLength()>0){bO.splice(0,1,bP.getItem(0));}
;}
;}
;break;case t:case g:if(!bN){return;}
;if(this.isNodeOpen(bM)){this.closeNode(bM);}
else {this.openNode(bM);}
;break;};}
;}
,_beforeApplySelection:function(bQ){if(bQ.length===0&&this.getSelectionMode()===u){var bR=this.__IS();var bS=this.getLookupTable().indexOf(bR);if(bS>=0){bQ.push(bS);}
;}
;}
,_afterApplySelection:function(){var bT=this.getSelection();if(bT.getLength()>0&&this.getSelectionMode()===u){this.__IR(bT.getItem(0));}
else {this.__IM=[];}
;}
,__IN:function(){this.buildLookupTable();this._applyDefaultSelection();}
...
;this.__IL=0;var bX=[];this.__IK=[];var bV=-1;var bW=this.getModel();if(bW!=null){if(!this.isHideRoot()){bV++;bX.push(bW);this.__IK.push(bV);}
;if(this.isNodeOpen(bW)){var bU=this.__IO(bW,bV);bX=bX.concat(bU);}
;}
;this._provider.removeBindings();this.__Ap.removeAll();this.__Ap.append(bX);this.__Dk();this._updateSelection();}
,__IO:function(bY,ca){var ce=[];ca++;if(!this.isNode(bY)){return ce;}
;var ch=bY.get(this.getChildProperty());if(ch==null){return ce;}
;ch=ch.copy();var cf=this.getDelegate();var cd=qx.util.Delegate.getMethod(cf,"filter");var cc=qx.util.Delegate.getMethod(cf,"sorter");if(cc!=null){ch.sort(cc);}
;for(var i=0;i<ch.getLength();i++){var cg=ch.getItem(i);if(cd&&!cd(cg)){continue;}
;if(this.isNode(cg)){this.__IK.push(ca);ce.push(cg);if(this.isNodeOpen(cg)){var cb=this.__IO(cg,ca);ce=ce.concat(cb);}
;}
else {if(this.isShowLeafs()){this.__IK.push(ca);ce.push(cg);}
;}
;}
;ch.dispose();return ce;}
,__IP:function(ci){if(!qx.lang.Array.contains(this.__IJ,ci)){this.__IJ.push(ci);this.fireDataEvent("open",ci);}
;}
,__IQ:function(cj,ck){if(cj===ck){this.__IP(ck);return true;}
;if(!this.isNode(cj)){return false;}
;var cm=cj.get(this.getChildProperty());if(cm==null){return false;}
;for(var i=0;i<cm.getLength();i++){var cn=cm.getItem(i);var cl=this.__IQ(cn,ck);if(cl===true){this.__IP(cn);return true;}
;}
;return false;}
,__Dk:function(){this.getPane().getRowConfig().setItemCount(this.__Ap.getLength());this.getPane().fullUpdate();}
,getParent:function(co){var cq=this.__Ap.indexOf(co);if(cq<0){return null;}
;var cr=this.__IK[cq];while(cq>0){cq--;var cp=this.__IK[cq];if(cp<cr){return this.__Ap.getItem(cq);}
;}
;return null;}
,__IR:function(cs){this.__IM=[];var parent=this.getParent(cs);while(parent!=null){this.__IM.unshift(parent);parent=this.getParent(parent);}
;}
,__IS:function(){if(this.__IM==null){return this.getModel();}
;var ct=this.getLookupTable();var parent=this.__IM.pop();while(parent!=null){if(ct.contains(parent)){return parent;}
;parent=this.__IM.pop();}
;return this.getModel();}
},destruct:function(){var cv=this.getPane();if(cv!=null){if(cv.hasListener(w)){cv.removeListener(w,this._onOpen,this);}
;if(cv.hasListener(D)){cv.removeListener(D,this._onOpen,this);}
;}
;if(!qx.core.ObjectRegistry.inShutDown&&this.__gg!=null){this.__gg.cancel();this.__gg.dispose();}
;var cu=this.getModel();if(cu!=null){cu.removeListener(C,this._onChangeBubble,this);}
;this._layer.removeListener(y,this._onUpdated,this);this._layer.destroy();this._provider.dispose();this.__Ap.dispose();this._layer=this._provider=this.__Ap=this.__IJ=this.__gg=null;}
});}
)();
...
(function(){var q="model",p="changeDelegate",o="virtual-tree-file",n="label",m="appearance",l="icon",k="bindItem",j=".length",i="]",h="virtual-tree-folder",c="[",g=".",f="qx.ui.tree.core.MWidgetController",b="BindingIds",a="",d="String";qx.Mixin.define(f,{construct:function(){this.__Dw=[];}
...
;}
,bindProperty:function(u,v,w,x,y){var z=this.__Dx(y,u);var A=this._tree.getLookupTable();var B=A.bind(z,x,v,w);this.__AP(x,B);}
,bindPropertyReverse:function(C,D,E,F,G){var H=this.__Dx(G,C);var I=this._tree.getLookupTable();var J=F.bind(D,I,H,E);this.__AP(F,J);}
,removeBindings:function(){while(this.__Dw.length>0){var K=this.__Dw.pop();this._removeBindingsFrom(K);}
;}
...
;}
,_removeBindingsFrom:function(O){var P=this.__Dy(O);while(P.length>0){var Q=P.pop();try{this._tree.getLookupTable().removeBinding(Q);}
catch(e){O.removeBinding(Q);}
;}
;if(qx.lang.Array.contains(this.__Dw,O)){qx.lang.Array.remove(this.__Dw,O);}
;}
,__Dx:function(R,S){var T=c+R+i;if(S!=null&&S!=a){T+=g+S;}
;return T;}
,__AP:function(U,V){var W=this.__Dy(U);if(!qx.lang.Array.contains(W,V)){W.push(V);}
;if(!qx.lang.Array.contains(this.__Dw,U)){this.__Dw.push(U);}
;}
,__Dy:function(X){var Y=X.getUserData(b);if(Y==null){Y=[];X.setUserData(b,Y);}
;return Y;}
...
;var p=this._renderer.getCellWidget();p.setOpen(q&&this._tree.isNodeOpen(o));p.setUserData(f,q);p.addListener(a,this.__IT,this);if(this._tree.getSelection().contains(o)){this._styleSelectabled(p);}
...
,poolCellWidget:function(s){s.removeListener(a,this.__IT,this);this._removeBindingsFrom(s);this._renderer.pool(s);this._onPool(s);}
...
,__IT:function(event){var I=event.getTarget();var J=I.getUserData("cell.row");var H=this._tree.getLookupTable().getItem(J);if(event.getData()){this._tree.openNode(H);}
