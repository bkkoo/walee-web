...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.ui.FiniteStateMachine.e681842e7a37.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
(function(){var h="qx.util.fsm.FiniteStateMachine",g="string",f="__Ht",d="__Hu",c="Number",b="String";qx.Class.define(h,{extend:qx.core.Object,construct:function(j){qx.core.Object.call(this);this.setName(j);this.__jr={};this.__Hr=null;this.__Hs=[];this.__Ht=[];this.__Hu=[];this.__Hv={};this.__Hw={};this.__Hx={};this.__Hy={};this.__Hz={};}
,statics:{StateChange:{CURRENT_STATE:1,POP_STATE_STACK:2,TERMINATE:3},EventHandling:{PREDICATE:1,BLOCKED:2},DebugFlags:{EVENTS:1,TRANSITIONS:2,FUNCTION_DETAIL:4,OBJECT_NOT_FOUND:8}},properties:{name:{check:b,nullable:true},state:{check:b,nullable:true},previousState:{check:b,nullable:true},nextState:{check:b,nullable:true},maxSavedStates:{check:c,init:2},debugFlags:{check:c,init:7}},members:{__jr:null,__Hr:null,__Ht:null,__Hu:null,__Hs:null,__Hv:null,__Hw:null,__Hx:null,__Hy:null,__Hz:null,__HA:false,addState:function(k){if(!k instanceof qx.util.fsm.State){throw new Error("Invalid state: not an instance of "+"qx.util.fsm.State");}
;var l=k.getName();if(l in this.__jr){throw new Error("State "+l+" already exists");}
;if(this.__Hr==null){this.__Hr=l;}
;this.__jr[l]=k;}
...
;var p=m.getName();var o=this.__jr[p];this.__jr[p]=m;if(n){o._bNeedDispose=true;}
;return o;}
,addObject:function(q,r,s){var v=r.toHashCode();this.__Hw[q]=v;this.__Hx[v]=q;this.__Hv[q]=r;if(!s){return;}
;if(typeof (s)==g){s=[s];}
;for(var i=0;i<s.length;i++){var u=s[i];if(!this.__Hy[u]){this.__Hy[u]={};}
;this.__Hy[u][q]=true;if(!this.__Hz[q]){this.__Hz[q]=[];}
;this.__Hz[q].push(u);}
;}
,removeObject:function(w){var y;var x;var z;var A;y=this.__Hw[w];if(this.__Hz[w]){for(var i=0;i<this.__Hz[w].length;i++){x=this.__Hz[w][i];delete this.__Hy[x][w];A=true;for(z in this.__Hy[x]){A=false;break;}
;if(A){delete this.__Hy[x];}
;}
;delete this.__Hz[w];}
;delete this.__Hx[y];delete this.__Hw[w];delete this.__Hv[w];}
,getObject:function(B){return this.__Hv[B];}
,getFriendlyName:function(C){var D=C?C.toHashCode():null;return D?this.__Hx[D]:null;}
,getGroupObjects:function(E){var a=[];for(var name in this.__Hy[E]){a.push(name);}
;return a;}
,displayAllObjects:function(){for(var F in this.__Hw){var H=this.__Hw[F];var G=this.getObject(F);this.debug(F+" => "+H);this.debug("  "+H+" => "+this.__Hx[H]);this.debug("  "+F+" => "+this.getObject(F));this.debug("  "+this.getObject(F)+" => "+this.getFriendlyName(G));}
;}
,_getInternalData:function(){return ({"states":this.__jr,"startState":this.__Hr,"eventQueue":this.__Ht,"blockedEvents":this.__Hu,"savedStates":this.__Hs,"friendlyToObject":this.__Hv,"friendlyToHash":this.__Hw,"hashToFriendly":this.__Hx,"groupToFriendly":this.__Hy,"friendlyToGroups":this.__Hz});}
,start:function(){var J=this.__Hr;if(J==null){throw new Error("Machine started with no available states");}
...
;this.__jr[J].getAutoActionsBeforeOnentry()(this);if(I){this.debug(this.getName()+"#"+J+"#entry");}
;this.__jr[J].getOnentry()(this,null);if(I){this.debug(this.getName()+"#"+J+"#actionsAfterOnentry");}
;this.__jr[J].getAutoActionsAfterOnentry()(this);}
,pushState:function(K){if(this.__Hs.length>=this.getMaxSavedStates()){throw new Error("Saved-state stack is full");}
;if(K===true){this.__Hs.push(this.getState());}
else if(K){this.__Hs.push(K);}
else {this.__Hs.push(this.getPreviousState());}
;}
,popState:function(){if(this.__Hs.length==0){throw new Error("Saved-state stack is empty");}
;return this.__Hs.pop();}
,postponeEvent:function(event){this.__Hu.unshift(event);}
,enqueueEvent:function(event,L){if(L){this.__Ht.push(event);}
else {this.__Ht.unshift(event);}
...
;this.__HA=true;while(this.__Ht.length>0){var event=this.__Ht.pop();var U=this.__HC(event);if(U){event.dispose();}
;}
;this.__HA=false;}
,__HC:function(event){var X;var ba;var W;var bi;var bg;var e;var be;var V=this.getDebugFlags();var bh=V&qx.util.fsm.FiniteStateMachine.DebugFlags.EVENTS;var bb=V&qx.util.fsm.FiniteStateMachine.DebugFlags.TRANSITIONS;var bf=V&qx.util.fsm.FiniteStateMachine.DebugFlags.FUNCTION_DETAIL;var bd=V&qx.util.fsm.FiniteStateMachine.DebugFlags.OBJECT_NOT_FOUND;var Y=this.getFriendlyName(event.getTarget());if(bh){this.debug(this.getName()+": Process event: "+event.getType()+(Y?" on "+Y:""));}
;X=this.getState();bi=this.__jr[X];bg=bi.transitions;e=bi.getEvents()[event.getType()];if(!e){if(bh){this.debug(this.getName()+": Event '"+event.getType()+"'"+" not handled.  Ignoring.");}
;return true;}
...
;this.__Hu.unshift(event);return false;default:if(typeof (be)=="string"){if(bg[be]){var bc=bg[be];bg={};bg[be]=bc;}
else {throw new Error("Explicit transition "+be+" does not exist");}
;break;}
;};for(var t in bg){var bc=bg[t];switch(bc.getPredicate()(this,event)){case true:break;case false:continue;case null:return true;default:throw new Error("Transition "+X+":"+t+" returned a value other than "+"true, false, or null.");};ba=bc.getNextState();if(typeof (ba)=="string"){if(!ba in this.__jr){throw new Error("Attempt to transition to nonexistent state "+ba);}
;this.setNextState(ba);}
else {switch(ba){case qx.util.fsm.FiniteStateMachine.StateChange.CURRENT_STATE:ba=X;this.setNextState(ba);break;case qx.util.fsm.FiniteStateMachine.StateChange.POP_STATE_STACK:if(this.__Hs.length==0){throw new Error("Attempt to transition to POP_STATE_STACK "+"while state stack is empty.");}
;ba=this.__Hs.pop();this.setNextState(ba);break;default:throw new Error("Internal error: invalid nextState");break;};}
...
;bi=this.__jr[this.getNextState()];this.setPreviousState(X);this.setState(this.getNextState());this.setNextState(null);W=X;X=ba;ba=undefined;if(bf){this.debug(this.getName()+"#"+X+"#autoActionsBeforeOnentry");}
...
;bi.getAutoActionsAfterOnentry()(this);for(var i=0;i<this.__Hu.length;i++){e=this.__Hu.pop();this.__Ht.unshift(e);}
...
;return true;}
},destruct:function(){this._disposeArray(f);this._disposeArray(d);this.__Hs=this.__jr=null;}
});}
)();
(function(){var p="function",o="  var groupObjects = ",n="__HJ",m="__HK",l="parameters",k="__HD",h=")",d=" fsm.getObject('",c="groups",b="  for (var i = 0; i < groupObjects.length; i++)",Y="autoActionsAfterOnentry",X="fsm",W="');",V="autoActionsBeforeOnexit",U="autoActionsBeforeOnentry",T="onexit",S="__HH",R="__HF",Q="autoActionsAfterOnexit",P="  fsm.debug(ex);",w="    fsm.getObject(objName).",x=")(fsm)",u="events",v="__HG",s="__HE",t="__HI",q="    var objName = groupObjects[i];",r="').",y="objects",z="catch(ex)",G="  {",E="    fsm.getGroupObjects('",J="  }",I="qx.util.fsm.State",L="onentry",K="try",B=",",O="context",N="string",M='"',A="}",C="{",D="(",F=";",H="object";qx.Class.define(I,{extend:qx.core.Object,construct:function(ba,bb){var bd;qx.core.Object.call(this);this.setName(ba);if(typeof (bb)!=H){throw new Error("State info must be an object");}
;bd=bb.context||window;this.setUserData(O,bd);for(var bc in bb){switch(bc){case L:this.setOnentry(this.__HL(bb[bc],bd));break;case T:this.setOnexit(this.__HL(bb[bc],bd));break;case U:this.setAutoActionsBeforeOnentry(bb[bc]);break;case Y:this.setAutoActionsAfterOnentry(bb[bc]);break;case V:this.setAutoActionsBeforeOnexit(bb[bc]);break;case Q:this.setAutoActionsAfterOnexit(bb[bc]);break;case u:this.setEvents(bb[bc]);break;case O:break;default:this.setUserData(bc,bb[bc]);this.debug("State "+ba+": "+"Adding user-provided field to state: "+bc);break;};}
...
;var bl;var bm=K+C;var bj;var bh;for(var f in bf){var bi=bf[f];if(!bi instanceof Array){throw new Error("Invalid function request type: "+"expected array, found "+typeof (bi));}
...
;if(typeof (bk[j])==p){bl+=D+bk[j]+x;}
else if(typeof (bk[j])==N){bl+=M+bk[j]+M;}
else {bl+=bk[j];}
;}
;bl+=h;var a=bh[y];if(!a){a=[];}
...
;bm+=d+a[j]+r+bl+F;}
;var g=bh[c];if(g){if(!g instanceof Array){throw new Error("Invalid 'groups' list: expected array, got "+typeof (g));}
;for(j=0;j<g.length;j++){bm+=o+E+g[j]+W+b+G+q+w+bl+F+J;}
;}
;}
;}
;bm+=A+z+C+P+A;return qx.lang.Function.bind(new Function(X,bm),bg);}
},properties:{name:{transform:k,nullable:true},onentry:{transform:s,nullable:true,init:function(bn,event){}
},onexit:{transform:R,nullable:true,init:function(bo,event){}
},autoActionsBeforeOnentry:{transform:S,nullable:true,init:function(bp,event){}
},autoActionsAfterOnentry:{transform:t,nullable:true,init:function(bq,event){}
},autoActionsBeforeOnexit:{transform:n,nullable:true,init:function(br,event){}
},autoActionsAfterOnexit:{transform:m,nullable:true,init:function(bs,event){}
},events:{transform:v,nullable:true}},members:{__HD:function(bt){if(typeof (bt)!="string"||bt.length<1){throw new Error("Invalid state name");}
;return bt;}
,__HE:function(bu){switch(typeof (bu)){case "undefined":return function(bv,event){}
...
,__HF:function(bw){switch(typeof (bw)){case "undefined":return function(bx,event){}
...
,__HG:function(by){if(typeof (by)!="object"){throw new Error("events must be an object");}
...
;}
;return by;}
,__HH:function(bB){return qx.util.fsm.State._commonTransformAutoActions("autoActionsBeforeOnentry",bB,this.getUserData("context"));}
,__HI:function(bC){return qx.util.fsm.State._commonTransformAutoActions("autoActionsAfterOnentry",bC,this.getUserData("context"));}
,__HJ:function(bD){return qx.util.fsm.State._commonTransformAutoActions("autoActionsBeforeOnexit",bD,this.getUserData("context"));}
,__HK:function(bE){return qx.util.fsm.State._commonTransformAutoActions("autoActionsAfterOnexit",bE,this.getUserData("context"));}
,__HL:function(f,bF){if(typeof (f)=="function"){f=qx.lang.Function.bind(f,bF);}
;return f;}
...
(function(){var n="__HP",m="predicate",l="__HQ",k="nextState",j="autoActionsBeforeOntransition",i="__HO",h="String",g="__HN",e="__HM",d="autoActionsAfterOntransition",a="qx.util.fsm.Transition",c="ontransition",b="context";qx.Class.define(a,{extend:qx.core.Object,construct:function(o,p){var r;qx.core.Object.call(this);this.setName(o);r=p.context||window;this.setUserData(b,r);for(var q in p){switch(q){case m:this.setPredicate(this.__HL(p[q],r));break;case k:this.setNextState(p[q]);break;case j:this.setAutoActionsBeforeOntransition(this.__HL(p[q],r));break;case d:this.setAutoActionsAfterOntransition(this.__HL(p[q],r));break;case c:this.setOntransition(this.__HL(p[q],r));break;case b:break;default:this.setUserData(q,p[q]);this.debug("Transition "+o+": "+"Adding user-provided field to transition: "+q);break;};}
;}
,properties:{name:{check:h,nullable:true},predicate:{init:function(s,event){return true;}
,transform:e},nextState:{init:qx.util.fsm.FiniteStateMachine.StateChange.CURRENT_STATE,transform:g},autoActionsBeforeOntransition:{init:function(t,event){}
,transform:i},autoActionsAfterOntransition:{init:function(u,event){}
,transform:n},ontransition:{init:function(v,event){}
,transform:l}},members:{__HM:function(w){switch(typeof (w)){case "undefined":return function(x,event){return true;}
...
,__HN:function(z){switch(typeof (z)){case "string":return z;case "number":switch(z){case qx.util.fsm.FiniteStateMachine.StateChange.CURRENT_STATE:case qx.util.fsm.FiniteStateMachine.StateChange.POP_STATE_STACK:case qx.util.fsm.FiniteStateMachine.StateChange.TERMINATE:return z;default:throw new Error("Invalid transition nextState value: "+z+": "+"nextState must be an explicit state name, "+"or one of the Fsm.StateChange constants");};break;default:throw new Error("Invalid transition nextState type: "+typeof (z));break;};}
,__HO:function(A){return qx.util.fsm.State._commonTransformAutoActions("autoActionsBeforeOntransition",A,this.getUserData("context"));}
,__HP:function(B){return qx.util.fsm.State._commonTransformAutoActions("autoActionsAfterOntransition",B,this.getUserData("context"));}
,__HQ:function(C){switch(typeof (C)){case "undefined":return function(D,event){}
...
,__HL:function(f,E){if(typeof (f)=="function"){f=qx.lang.Function.bind(f,E);}
;return f;}
}});}
)();
