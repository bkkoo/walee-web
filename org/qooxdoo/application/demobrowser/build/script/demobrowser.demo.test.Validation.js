...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.test.Validation.a033c78957a8.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
;O.setHtml(O.getHtml()+a+b);N.setGender(u);O.setHtml(O.getHtml()+A+N.getGender()+B);O.setHtml(O.getHtml()+M+r);try{N.setAge(d);O.setHtml(O.getHtml()+A+N.getGender()+B);}
...
}});qx.Class.define(E,{extend:qx.core.Object,properties:{name:{init:s,validate:L},age:{init:18,validate:qx.util.Validate.number()},gender:{init:z,validate:qx.util.Validate.inArray([z,u])}},members:{__GQ:function(T){if(T.length<2){throw new qx.core.ValidationError("Validation Error: Name "+"must be longer than one characters. ("+T+")");}
;}
}});}
)();
