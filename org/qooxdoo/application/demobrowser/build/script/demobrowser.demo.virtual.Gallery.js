...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.virtual.Gallery.ed3aa46b30f4.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
;}
}});qx.Class.define(g,{extend:demobrowser.demo.virtual.AbstractGallery,construct:function(bv){demobrowser.demo.virtual.AbstractGallery.call(this,bv);this.__IC=new demobrowser.demo.virtual.GalleryCell();}
,members:{__IC:null,_createScroller:function(){var bw=new qx.ui.virtual.core.Scroller(1,this.itemPerLine,this.itemHeight,this.itemWidth);this.layer=new qx.ui.virtual.layer.WidgetCell(this);bw.getPane().addLayer(this.layer);new qx.ui.virtual.behavior.Prefetch(bw,{minLeft:0,maxLeft:0,minRight:0,maxRight:0,minAbove:200,maxAbove:300,minBelow:600,maxBelow:800}).set({interval:500});return bw;}
...
;if(bz){this.__IC.updateStates(bB,{selected:1});}
else {this.__IC.updateStates(bB,{});}
;}
;}
...
;var bH=this.__IC.getCellWidget(bI,bG);bH.setUserData(x,bF);return bH;}
,poolCellWidget:function(bJ){this.__IC.pool(bJ);}
},destruct:function(){this._disposeObjects(I,H);}
...
(function(){var l="_applyInterval",k="appear",j="qx.debug",i="interval",h="qx.ui.virtual.behavior.Prefetch",g="_applyScroller",f="qx.ui.virtual.core.Scroller",e="Integer",d="__jQ",c="scroll",a="scrollbar-x",b="scrollbar-y";qx.Class.define(h,{extend:qx.core.Object,construct:function(m,n){if(qx.core.Environment.get(j)){this.assertObject(n);this.assertPositiveInteger(n.minLeft);this.assertPositiveInteger(n.maxLeft);this.assertPositiveInteger(n.minRight);this.assertPositiveInteger(n.maxRight);this.assertPositiveInteger(n.minAbove);this.assertPositiveInteger(n.maxAbove);this.assertPositiveInteger(n.minBelow);this.assertPositiveInteger(n.maxBelow);}
;qx.core.Object.call(this);this.setPrefetchX(n.minLeft,n.maxLeft,n.minRight,n.maxRight);this.setPrefetchY(n.minAbove,n.maxAbove,n.minBelow,n.maxBelow);this.__jQ=new qx.event.Timer(this.getInterval());this.__jQ.addListener(i,this._onInterval,this);if(m){this.setScroller(m);}
;}
,properties:{scroller:{check:f,nullable:true,init:null,apply:g},interval:{check:e,init:200,apply:l}},members:{__GW:null,__GX:null,__jQ:null,__GY:null,__Ha:null,setPrefetchX:function(o,p,q,r){this.__GW=[o,p,q,r];}
,setPrefetchY:function(s,t,u,v){this.__GX=[s,t,u,v];}
,_onInterval:function(){var w=this.__GW;if(w[1]&&w[3]){this.getScroller().getPane().prefetchX(w[0],w[1],w[2],w[3]);qx.ui.core.queue.Manager.flush();}
;var x=this.__GX;if(x[1]&&x[3]){this.getScroller().getPane().prefetchY(x[0],x[1],x[2],x[3]);qx.ui.core.queue.Manager.flush();}
;}
,_applyScroller:function(y,z){if(z){if(this.__GY){z.getChildControl(a).removeListenerById(this.__GY);}
;if(this.__Ha){z.getChildControl(b).removeListenerById(this.__Ha);}
;}
;if(y){if(!y.getContainerElement().getDomElement()){this.__jQ.stop();y.addListenerOnce(k,this.__jQ.start,this.__jQ);}
else {this.__jQ.restart();}
;this.__GY=y.getChildControl(a).addListener(c,this.__jQ.restart,this.__jQ);this.__Ha=y.getChildControl(b).addListener(c,this.__jQ.restart,this.__jQ);}
else {this.__jQ.stop();}
;}
,_applyInterval:function(A,B){this.__jQ.setInterval(A);}
},destruct:function(){this.setScroller(null);this.__GW=this.__GX=null;this._disposeObjects(d);}
});}
)();
...
;this.__Iv=[];this.__Iw=[];this._isHorizontal=(v||g)==g;}
,properties:{defaultLineColor:{init:r,check:e,themeable:true},defaultLineSize:{init:1,check:k,themeable:true}},members:{__Iv:null,__Iw:null,isHorizontal:function(){return this._isHorizontal;}
...
;this.__Iv[A]=B;if(this.__Ix(A)){this.updateLayerData();}
;}
...
;this.__Iw[C]=D;if(this.__Ix(C)){this.updateLayerData();}
;}
,__Ix:function(E){if(this._isHorizontal){var I=this.getFirstColumn();var G=I+this.getColumnSizes().length-1;return E>=I&&E<=G;}
else {var H=this.getFirstRow();var F=H+this.getRowSizes().length-1;return E>=H&&E<=F;}
;}
,getLineSize:function(J){return this.__Iw[J]||this.getDefaultLineSize();}
,getLineColor:function(K){return this.__Iv[K]||this.getDefaultLineColor();}
,__Iy:function(L,M,N){var top=0;var O,P;for(var y=0;y<N.length-1;y++){O=this.getLineColor(M+y);P=this.getLineSize(M+y);top+=N[y];L.push(a,f,s+P+d,q,u,top-((P>1)?Math.floor(P/2):1),d,b,O,j,c);}
;}
,__Iz:function(Q,R,S){var V=0;var T,U;for(var x=0;x<S.length-1;x++){T=this.getLineColor(R+x);U=this.getLineSize(R+x);V+=S[x];Q.push(a,f,p+U+d,o,h,m,V-((U>1)?Math.floor(U/2):1),d,b,T,j,c);}
;}
,_fullUpdate:function(W,X,Y,ba){var bb=[];if(this._isHorizontal){this.__Iy(bb,W,Y);}
else {this.__Iz(bb,X,ba);}
