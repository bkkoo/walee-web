...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.test.Decorator_Box.ec63af621fb2.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
(function(){var s="sliceBottom",r="horizontal",q="qx.debug.dispose",p="qx.ui.decoration.AbstractBox",o="-l",n="qx.debug",m="sliceLeft",l="_applyBaseImage",k="-b",j="sliceRight",d="_applyFill",i="shorthand",g="-t",c="-r",b="String",f="sliceTop",e="set",h="Number",a="_applySlices";qx.Class.define(p,{extend:qx.ui.decoration.Abstract,implement:[qx.ui.decoration.IDecorator],construct:function(t,u,v){qx.ui.decoration.Abstract.call(this);this._setOrientation(v);if(qx.ui.decoration.css3.BorderImage.IS_SUPPORTED){this.__vF=new qx.ui.decoration.css3.BorderImage();if(t){this.__vG(t,v);}
;if(u!=null){this.__vF.setInsets(u);}
;}
else {this.__vF=new qx.ui.decoration.BoxDiv(t,u,v);}
;if(qx.core.Environment.get(q)){this.__vF.$$ignoreDisposeWarning=true;}
;}
,properties:{baseImage:{check:b,nullable:true,apply:l},sliceLeft:{check:h,nullable:true,apply:a},sliceRight:{check:h,nullable:true,apply:a},sliceBottom:{check:h,nullable:true,apply:a},sliceTop:{check:h,nullable:true,apply:a},slices:{group:[f,j,s,m],mode:i},fill:{apply:d}},members:{__vF:null,_isHorizontal:null,_setOrientation:function(w){this._isHorizontal=w==r;}
,getMarkup:function(){return this.__vF.getMarkup();}
,resize:function(x,y,z){this.__vF.resize(x,y,z);}
,tint:function(A,B){}
,getInsets:function(){return this.__vF.getInsets();}
,_applyInsets:function(C,D,name){var E=e+qx.lang.String.firstUp(name);this.__vF[E](C);}
,_applySlices:function(F,G,name){var H=e+qx.lang.String.firstUp(name);if(this.__vF[H]){this.__vF[H](F);}
;}
,_applyFill:function(I,J,name){if(this.__vF.setFill){this.__vF.setFill(I);}
;}
,_applyBaseImage:function(K,L){if(this.__vF instanceof qx.ui.decoration.BoxDiv){this.__vF.setBaseImage(K);}
else {this.__vG(K);}
;}
,__vG:function(M){this.__vF.setBorderImage(M);var S=qx.util.AliasManager.getInstance().resolve(M);var T=/(.*)(\.[a-z]+)$/.exec(S);var Q=T[1];var R=T[2];var P=qx.util.ResourceManager.getInstance();if(qx.core.Environment.get(n)){if(!P.has(S)){this.warn("The border image '"+S+"' is not available!");}
;}
;if(this._isHorizontal){var V=P.getImageWidth(Q+o+R);var N=P.getImageWidth(Q+c+R);if(N&&V){this.__vF.setSlice([0,N,0,V]);}
;}
else {var O=P.getImageHeight(Q+k+R);var U=P.getImageHeight(Q+g+R);if(U&&O){this.__vF.setSlice([U,0,O,0]);}
;}
;}
},destruct:function(){this.__vE=this.__GC=this.__CB=null;this.__vF.dispose();}
});}
)();
...
;}
,properties:{baseImage:{check:w,nullable:true,apply:c}},members:{__vE:null,__GC:null,__CB:null,_getDefaultInsets:function(){return {top:0,right:0,bottom:0,left:0};}
,_isInitialized:function(){return !!this.__vE;}
,_setOrientation:function(A){this._isHorizontal=A==q;}
,getMarkup:function(){if(this.__vE){return this.__vE;}
;var B=qx.bom.element.Decoration;var C=this.__GC;var D=this.__CB;var E=[];E.push(e);if(this._isHorizontal){E.push(B.create(C.l,m,{top:0,left:0}));E.push(B.create(C.c,p,{top:0,left:D.left+k}));E.push(B.create(C.r,m,{top:0,right:0}));}
...
;E.push(i);return this.__vE=E.join(v);}
,resize:function(F,G,H){F.style.width=G+k;F.style.height=H+k;var I=this.__CB;if(this._isHorizontal){var innerWidth=G-I.left-I.right;innerWidth=innerWidth<0?0:innerWidth;F.childNodes[1].style.width=innerWidth+k;}
...
;}
,tint:function(J,K){}
,_applyBaseImage:function(L,M){if(qx.core.Environment.get(d)){if(this.__vE){throw new Error("This decorator is already in-use. Modification is not possible anymore!");}
;}
;var N=qx.util.ResourceManager.getInstance();if(L){var P=qx.util.AliasManager.getInstance();var R=P.resolve(L);var S=/(.*)(\.[a-z]+)$/.exec(R);var Q=S[1];var O=S[2];var T=this.__GC={t:Q+h+O,b:Q+b+O,c:Q+t+O,l:Q+j+O,r:Q+f+O};this.__CB={top:N.getImageHeight(T.t),bottom:N.getImageHeight(T.b),left:N.getImageWidth(T.l),right:N.getImageWidth(T.r)};}
;}
},destruct:function(){this.__vE=this.__GC=this.__CB=null;}
});}
)();
