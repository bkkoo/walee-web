...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.layout.HBox_NegativeMargin.7c071834e402.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
;}
,properties:{sort:{check:[x,v,n],init:x,apply:c},separatorX:{check:u,nullable:true,apply:l},separatorY:{check:u,nullable:true,apply:l},connectSeparators:{check:k,init:false,apply:l},spacingX:{check:a,init:0,apply:l},spacingY:{check:a,init:0,apply:l}},members:{__fU:null,__CB:null,verifyLayoutProperty:qx.core.Environment.select(g,{"true":function(C,name,D){this.assertInArray(name,[o,q,j,f],h+name+e);if(name===q){this.assertInArray(D,[t,p,b,r,w]);}
...
,__CC:{north:1,south:2,west:3,east:4,center:5},__CD:{'1':"top",'2':"bottom",'3':"left",'4':"right"},__lV:function(){var E=this._getLayoutChildren();var M,G;var length=E.length;var I=[];var L=[];var J=[];var F=this.getSort()===v;var K=this.getSort()===n;for(var i=0;i<length;i++){M=E[i];J=M.getLayoutProperties().edge;if(J===w){if(G){throw new Error("It is not allowed to have more than one child aligned to 'center'!");}
;G=M;}
...
;this.__fU=N;var H=[];for(var i=0;i<length;i++){J=N[i].getLayoutProperties().edge;H[i]=this.__CC[J]||5;}
;this.__CB=H;delete this._invalidChildrenCache;}
,renderLayout:function(O,P){if(this._invalidChildrenCache){this.__lV();}
;var bj=qx.ui.layout.Util;var V=this.__fU;var bk=this.__CB;var length=V.length;var Y,R,X,be,bf,bp,bc,S,bm;var bt=[];var bb=[];var bs=this._getSeparatorWidths();var bx=this.getSpacingX();var bw=this.getSpacingY();var W=-bx;var bl=-bw;if(bs.x){W-=bs.x+bx;}
;if(bs.y){bl-=bs.y+bw;}
...
;}
;this._clearSeparators();var bq=this.getSeparatorX(),br=this.getSeparatorY();var bu=this.getConnectSeparators();var bh=0,bo=0;var bB,top,bc,S,bg,by;var bi,bv,bA,Q;var bz,ba,bd,U;var bn=this.__CD;for(var i=0;i<length;i++){R=V[i];by=bk[i];X=R.getSizeHint();bz=R.getMarginTop();ba=R.getMarginBottom();bd=R.getMarginLeft();U=R.getMarginRight();switch(by){case 1:case 2:bc=O-bd-U;if(bc<X.minWidth){bc=X.minWidth;}
else if(bc>X.maxWidth){bc=X.maxWidth;}
...
;var bO=this.__fU;var bY=this.__CB;var length=bO.length;var bS,ca;var bU,bT;var bV=0,ch=0;var bL=0,cc=0;var bW=0,cg=0;var bM=0,cb=0;var cd=this._getSeparatorWidths();var cf=this.getSpacingX(),ce=this.getSpacingY();var bQ=-cf,bP=-ce;if(cd.x){bQ-=cd.x+cf;}
;if(cd.y){bP-=cd.y+ce;}
...
},destruct:function(){this.__CB=this.__fU=null;}
});}
)();
